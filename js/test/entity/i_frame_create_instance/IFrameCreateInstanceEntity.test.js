...
describe('IFrameCreateInstanceEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        i_frame_create_instance: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.IFrameCreateInstance().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.IFrameCreateInstance().stream('list')) {
