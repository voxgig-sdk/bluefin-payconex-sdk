...
describe('AccountUpdaterEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        account_updater: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.AccountUpdater().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.AccountUpdater().stream('list')) {
  ...
  test('basic', async () => {
    ...
    let account_updater_ref01_data = Object.values(setup.data.existing.account_updater)[0]
