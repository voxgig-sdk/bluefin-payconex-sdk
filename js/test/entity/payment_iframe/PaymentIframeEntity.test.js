...
describe('PaymentIframeEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        payment_iframe: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.PaymentIframe().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.PaymentIframe().stream('list')) {
  ...
  test('basic', async () => {
    ...
    let payment_iframe_ref01_data = Object.values(setup.data.existing.payment_iframe)[0]
