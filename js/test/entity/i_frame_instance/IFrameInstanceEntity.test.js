...
describe('IFrameInstanceEntity', async () => {

  test('instance', async () => {
    const testsdk = BluefinPayconexSDK.test()
    const ent = testsdk.IFrameInstance()
    assert(null != ent)
  })
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        i_frame_instance: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.IFrameInstance().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.IFrameInstance().stream('list')) {
  ...
  test('basic', async () => {
    ...
    let i_frame_instance_ref01_data = Object.values(setup.data.existing.i_frame_instance)[0]

    // LOAD
    const i_frame_instance_ref01_ent = client.IFrameInstance()
    const i_frame_instance_ref01_match_dt0 = {}
    const i_frame_instance_ref01_data_dt0 = await i_frame_instance_ref01_ent.load(i_frame_instance_ref01_match_dt0)
    assert(null != i_frame_instance_ref01_data_dt0)
