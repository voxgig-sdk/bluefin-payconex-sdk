...
describe('ApplePaySessionEntity', async () => {

  test('instance', async () => {
    const testsdk = BluefinPayconexSDK.test()
    const ent = testsdk.ApplePaySession()
    assert(null != ent)
  })
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        apple_pay_session: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.ApplePaySession().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.ApplePaySession().stream('list')) {
  ...
  test('basic', async () => {
    ...
    // CREATE
    const apple_pay_session_ref01_ent = client.ApplePaySession()
    let apple_pay_session_ref01_data = setup.data.new.apple_pay_session['apple_pay_session_ref01']
    ...
    assert(null != apple_pay_session_ref01_data)
