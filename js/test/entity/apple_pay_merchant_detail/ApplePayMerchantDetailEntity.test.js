...
describe('ApplePayMerchantDetailEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        apple_pay_merchant_detail: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.ApplePayMerchantDetail().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.ApplePayMerchantDetail().stream('list')) {
  ...
  test('basic', async () => {
    ...
    // LIST
    const apple_pay_merchant_detail_ref01_match = {}
