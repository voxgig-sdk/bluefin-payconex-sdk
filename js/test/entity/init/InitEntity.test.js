...
describe('InitEntity', async () => {

  test('instance', async () => {
    const testsdk = BluefinPayconexSDK.test()
    const ent = testsdk.Init()
    assert(null != ent)
  })
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        init: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.Init().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.Init().stream('list')) {
  ...
  test('basic', async () => {
    ...
    // CREATE
    const init_ref01_ent = client.Init()
    let init_ref01_data = setup.data.new.init['init_ref01']
    init_ref01_data['account_id'] = setup.idmap['account01']

    init_ref01_data = await init_ref01_ent.create(init_ref01_data)
    assert(null != init_ref01_data)
