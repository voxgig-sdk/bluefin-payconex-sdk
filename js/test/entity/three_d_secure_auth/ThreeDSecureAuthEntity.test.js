...
describe('ThreeDSecureAuthEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        three_d_secure_auth: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.ThreeDSecureAuth().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.ThreeDSecureAuth().stream('list')) {
