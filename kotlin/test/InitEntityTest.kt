...
class InitEntityTest {
  ...
  @Test
  fun basic() {
    val setup = initBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "init.$op", mode)
    ...
    var initRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.init"), "init_ref01")) ?: linkedMapOf())
    initRef01Data["account_id"] = setup.idmap!!["account01"]

    val initRef01DataResult = initRef01Ent.create(initRef01Data, null)
    initRef01Data = Helpers.toMapAny(initRef01DataResult) ?: linkedMapOf()
    assertNotNull(initRef01Data, "expected create result to be a map")

  }

  companion object {
    fun initBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read init test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFINPAYCONEX_TEST_INIT_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFINPAYCONEX_TEST_INIT_ENTID"] = idmap
      envm["BLUEFINPAYCONEX_TEST_LIVE"] = "FALSE"
      envm["BLUEFINPAYCONEX_TEST_EXPLAIN"] = "FALSE"
      envm["BLUEFINPAYCONEX_APIKEY"] = "NONE"
      val env = RunnerSupport.envOverride(envm)

      var idmapResolved = Helpers.toMapAny(env["BLUEFINPAYCONEX_TEST_INIT_ENTID"])
