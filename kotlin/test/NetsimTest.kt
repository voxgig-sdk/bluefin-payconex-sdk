...
class NetsimTest {
  ...
  fun offlineSimulationFailsRequest() {
    val client = BluefinPayconexSDK.testSDK(fhMap("net", fhMap("offline", true)), null)
  ...
  fun failstatusSimulationSurfacesStatus() {
    val client = BluefinPayconexSDK.testSDK(fhMap("net", fhMap("failTimes", 1, "failStatus", 503)), null)
    val res = client.direct(fhMap("path", "/ping"))
  ...
  fun latencySimulationDelaysRequest() {
    val delay = 60
    val client = BluefinPayconexSDK.testSDK(fhMap("net", fhMap("latency", delay)), null)
