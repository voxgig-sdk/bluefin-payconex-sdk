...
class ApplePaySessionEntityTest {
  ...
  @Test
  fun basic() {
    val setup = applePaySessionBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "apple_pay_session.$op", mode)
    ...
    var applePaySessionRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.apple_pay_session"), "apple_pay_session_ref01")) ?: linkedMapOf())
    ...
    applePaySessionRef01Data = Helpers.toMapAny(applePaySessionRef01DataResult) ?: linkedMapOf()
    assertNotNull(applePaySessionRef01Data, "expected create result to be a map")

  }

  companion object {
    fun applePaySessionBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read apple_pay_session test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID"] = idmap
