...
class DynamicDescriptorDirectTest {

  @Test
  fun directListDynamicDescriptor() {
    val mockres = mutableListOf<Any?>()
    mockres.add(jm("id", "direct01"))
    mockres.add(jm("id", "direct02"))
    val setup = directSetup(mockres)
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-list-dynamic_descriptor", mode)
    Assumptions.assumeTrue(
      reason == null,
      if (reason == null || "" == reason) "skipped via sdk-test-control.json" else reason,
    )
    if (setup.live) {
      for (liveKey in arrayOf("account01")) {
  ...
  @Test
  fun directLoadDynamicDescriptor() {
    val setup = directSetup(jm("id", "direct01"))
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-load-dynamic_descriptor", mode)
    Assumptions.assumeTrue(
      reason == null,
      if (reason == null || "" == reason) "skipped via sdk-test-control.json" else reason,
    )
  ...
  companion object {
    ...
    class DirectSetup {
      lateinit var client: BluefinPayconexSDK
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFINPAYCONEX_TEST_DYNAMIC_DESCRIPTOR_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        mergedOpts["apikey"] = env["BLUEFINPAYCONEX_APIKEY"]
        setup.client = BluefinPayconexSDK(mergedOpts)
        setup.live = true

        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFINPAYCONEX_TEST_DYNAMIC_DESCRIPTOR_ENTID"]
      ...
      setup.client = BluefinPayconexSDK(jm(
