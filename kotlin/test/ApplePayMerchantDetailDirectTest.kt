...
class ApplePayMerchantDetailDirectTest {

  @Test
  fun directListApplePayMerchantDetail() {
    val mockres = mutableListOf<Any?>()
    mockres.add(jm("id", "direct01"))
    mockres.add(jm("id", "direct02"))
    val setup = directSetup(mockres)
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-list-apple_pay_merchant_detail", mode)
  ...
  companion object {
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFINPAYCONEX_TEST_APPLE_PAY_MERCHANT_DETAIL_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFINPAYCONEX_TEST_APPLE_PAY_MERCHANT_DETAIL_ENTID"]
