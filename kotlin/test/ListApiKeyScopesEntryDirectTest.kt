...
class ListApiKeyScopesEntryDirectTest {

  @Test
  fun directListListApiKeyScopesEntry() {
    val mockres = mutableListOf<Any?>()
    mockres.add(jm("id", "direct01"))
    mockres.add(jm("id", "direct02"))
    val setup = directSetup(mockres)
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-list-list_api_key_scopes_entry", mode)
  ...
  companion object {
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFINPAYCONEX_TEST_LIST_API_KEY_SCOPES_ENTRY_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFINPAYCONEX_TEST_LIST_API_KEY_SCOPES_ENTRY_ENTID"]
