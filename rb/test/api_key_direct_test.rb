...
class ApiKeyDirectTest < Minitest::Test
  def test_direct_list_api_key
    setup = api_key_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-api_key", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_api_key
    setup = api_key_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-api_key", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def api_key_direct_setup(mockres)
