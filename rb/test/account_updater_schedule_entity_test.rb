...
class AccountUpdaterScheduleEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = account_updater_schedule_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "account_updater_schedule." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_SCHEDULE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    account_updater_schedule_ref01_ent = client.AccountUpdaterSchedule(nil)
    ...
    account_updater_schedule_ref01_data_result = account_updater_schedule_ref01_ent.create(account_updater_schedule_ref01_data, nil)
    account_updater_schedule_ref01_data = Helpers.to_map(account_updater_schedule_ref01_data_result)
    assert !account_updater_schedule_ref01_data.nil?

  end
end

def account_updater_schedule_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "account_updater_schedule", "AccountUpdaterScheduleTestData.json")
  ...
  entid_env_raw = ENV["BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_SCHEDULE_ENTID"]
