...
class ListApiKeyScopesEntryDirectTest < Minitest::Test
  def test_direct_list_list_api_key_scopes_entry
    setup = list_api_key_scopes_entry_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-list_api_key_scopes_entry", setup[:live] ? "live" : "unit")
    if _should_skip
...
def list_api_key_scopes_entry_direct_setup(mockres)
