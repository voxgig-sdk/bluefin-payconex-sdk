...
class ApplePaySessionEntityTest < Minitest::Test
  def test_create_instance
    testsdk = BluefinPayconexSDK.test(nil, nil)
    ent = testsdk.ApplePaySession(nil)
    assert !ent.nil?
  end

  def test_basic_flow
    setup = apple_pay_session_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "apple_pay_session." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    apple_pay_session_ref01_ent = client.ApplePaySession(nil)
    ...
    apple_pay_session_ref01_data_result = apple_pay_session_ref01_ent.create(apple_pay_session_ref01_data, nil)
    apple_pay_session_ref01_data = Helpers.to_map(apple_pay_session_ref01_data_result)
    assert !apple_pay_session_ref01_data.nil?

  end
end

def apple_pay_session_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "apple_pay_session", "ApplePaySessionTestData.json")
  ...
  entid_env_raw = ENV["BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID"]
