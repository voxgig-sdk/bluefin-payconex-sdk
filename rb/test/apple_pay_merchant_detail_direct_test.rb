...
class ApplePayMerchantDetailDirectTest < Minitest::Test
  def test_direct_list_apple_pay_merchant_detail
    setup = apple_pay_merchant_detail_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-apple_pay_merchant_detail", setup[:live] ? "live" : "unit")
    if _should_skip
...
def apple_pay_merchant_detail_direct_setup(mockres)
