...
class AccountUpdaterUpdateDirectTest < Minitest::Test
  def test_direct_load_account_updater_update
    setup = account_updater_update_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-account_updater_update", setup[:live] ? "live" : "unit")
    if _should_skip
...
def account_updater_update_direct_setup(mockres)
