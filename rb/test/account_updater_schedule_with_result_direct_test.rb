...
class AccountUpdaterScheduleWithResultDirectTest < Minitest::Test
  def test_direct_list_account_updater_schedule_with_result
    setup = account_updater_schedule_with_result_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-account_updater_schedule_with_result", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["account01", "schedule01"].each do |_live_key|
...
def account_updater_schedule_with_result_direct_setup(mockres)
