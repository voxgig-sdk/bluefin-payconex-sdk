...
module BluefinPayconexUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinPayconexResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinPayconexResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinPayconexResponse.new(fetched)
    else
