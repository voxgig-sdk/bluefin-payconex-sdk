...
class DynamicDescriptorEntity
  ...
  def make
    opts = @_entopts.dup
    DynamicDescriptorEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinPayconexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [DynamicDescriptor, Hash] the current DynamicDescriptor data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinPayconexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of DynamicDescriptor fields)
  def match_get
  ...
  #
  # @param reqmatch [DynamicDescriptorLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.DynamicDescriptor.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [DynamicDescriptor, Hash] the loaded DynamicDescriptor; raises BluefinPayconexError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BluefinPayconexHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [DynamicDescriptorListMatch, Hash, nil] match filter (any subset of
  #   DynamicDescriptor fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<DynamicDescriptor>, Array] the matching DynamicDescriptor items; raises BluefinPayconexError on failure
  ...
  #
  # @param reqdata [DynamicDescriptorCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [DynamicDescriptor, Hash] the created DynamicDescriptor; raises BluefinPayconexError on failure
  ...
  #
  # @param reqdata [DynamicDescriptorUpdateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [DynamicDescriptor, Hash] the updated DynamicDescriptor; raises BluefinPayconexError on failure
  ...
  #
  # @param reqmatch [DynamicDescriptorRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [DynamicDescriptor, Hash] the removed DynamicDescriptor; raises BluefinPayconexError on failure
