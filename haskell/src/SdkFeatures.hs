-- BluefinPayconex SDK features + API-agnostic client helpers.
--
...
clienttrackFeature = do
  ...
  let nameOf = do opts <- readIORef options; nm <- optStr opts "clientName" "BluefinPayconex-SDK"; ver <- optStr opts "clientVersion" "0.0.1"; pure (nm ++ "/" ++ ver)
      genId kind = do
        opts <- readIORef options; v <- getp opts "idgen"
