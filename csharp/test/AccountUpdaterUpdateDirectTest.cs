// account_updater_update direct API tests (generated from the API model).
...
public class AccountUpdaterUpdateDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = AccountUpdaterUpdateDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-account_updater_update", _mode);
        if (_shouldSkip)
        {
    ...
    private class AccountUpdaterUpdateDirectSetupResult
    {
    ...
    private static AccountUpdaterUpdateDirectSetupResult AccountUpdaterUpdateDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_UPDATE_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_UPDATE_ENTID"];
            ...
            return new AccountUpdaterUpdateDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
