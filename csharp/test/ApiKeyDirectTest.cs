// api_key direct API tests (generated from the API model).
...
public class ApiKeyDirectTest
{
    ...
    {
        var setup = ApiKeyDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-api_key", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = ApiKeyDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-api_key", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
        ...
        if (setup.Live)
        {
            ...
            pathParams["id"] = "api_d193148ac2624de7b43a191682a8822f";
        }
        else
        {
            pathParams["account_id"] = "direct01";
    ...
    private class ApiKeyDirectSetupResult
    {
    ...
    private static ApiKeyDirectSetupResult ApiKeyDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINPAYCONEX_TEST_API_KEY_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINPAYCONEX_TEST_API_KEY_ENTID"];
            ...
            return new ApiKeyDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
