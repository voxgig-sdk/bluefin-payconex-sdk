// apple_pay_merchant_detail direct API tests (generated from the API model).
...
public class ApplePayMerchantDetailDirectTest
{
    [Fact]
    public void DirectList()
    {
        var setup = ApplePayMerchantDetailDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-apple_pay_merchant_detail", _mode);
        if (_shouldSkip)
        {
    ...
    private class ApplePayMerchantDetailDirectSetupResult
    {
    ...
    private static ApplePayMerchantDetailDirectSetupResult ApplePayMerchantDetailDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINPAYCONEX_TEST_APPLE_PAY_MERCHANT_DETAIL_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINPAYCONEX_TEST_APPLE_PAY_MERCHANT_DETAIL_ENTID"];
            ...
            return new ApplePayMerchantDetailDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
