// webhook direct API tests (generated from the API model).
...
public class WebhookDirectTest
{
    ...
    {
        var setup = WebhookDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-webhook", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = WebhookDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-webhook", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
        ...
        if (setup.Live)
        {
            ...
            pathParams["id"] = "wbh_d193148ac2624de7b43a191682a8822f";
        }
        else
        {
            pathParams["account_id"] = "direct01";
    ...
    private class WebhookDirectSetupResult
    {
    ...
    private static WebhookDirectSetupResult WebhookDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID"];
            ...
            return new WebhookDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
