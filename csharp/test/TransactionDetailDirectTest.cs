// transaction_detail direct API tests (generated from the API model).
...
public class TransactionDetailDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = TransactionDetailDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-transaction_detail", _mode);
        if (_shouldSkip)
        {
        ...
        if (setup.Live)
        {
            ...
            pathParams["transaction_id"] = 1861;
        }
        else
        {
            pathParams["account_id"] = "direct01";
            pathParams["transaction_id"] = "direct02";
        }
    ...
    private class TransactionDetailDirectSetupResult
    {
    ...
    private static TransactionDetailDirectSetupResult TransactionDetailDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINPAYCONEX_TEST_TRANSACTION_DETAIL_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINPAYCONEX_TEST_TRANSACTION_DETAIL_ENTID"];
            ...
            return new TransactionDetailDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
