// dynamic_descriptor direct API tests (generated from the API model).
...
public class DynamicDescriptorDirectTest
{
    ...
    {
        var setup = DynamicDescriptorDirectSetup(new List<object?>
        {
        ...
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-list-dynamic_descriptor", _mode);
        if (_shouldSkip)
        {
    ...
    {
        var setup = DynamicDescriptorDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-dynamic_descriptor", _mode);
        if (_shouldSkip)
        {
            return; // skipped via sdk-test-control.json
        }
        ...
        if (setup.Live)
        {
            pathParams["account_id"] = 180000027666;
            pathParams["id"] = "ddesc_1669911404412ae74cd5eef8f463495d6b";
        }
        else
        {
            pathParams["account_id"] = "direct01";
    ...
    private class DynamicDescriptorDirectSetupResult
    {
    ...
    private static DynamicDescriptorDirectSetupResult DynamicDescriptorDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFINPAYCONEX_TEST_DYNAMIC_DESCRIPTOR_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFINPAYCONEX_TEST_DYNAMIC_DESCRIPTOR_ENTID"];
            ...
            return new DynamicDescriptorDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
