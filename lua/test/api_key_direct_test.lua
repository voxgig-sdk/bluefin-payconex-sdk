...
describe("ApiKeyDirect", function()
  it("should direct-list-api_key", function()
    local setup = api_key_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-api_key", setup.live and "live" or "unit")
  ...
  it("should direct-load-api_key", function()
    local setup = api_key_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-api_key", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function api_key_direct_setup(mockres)
