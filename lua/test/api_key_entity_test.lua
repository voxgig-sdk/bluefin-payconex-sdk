...
describe("ApiKeyEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:ApiKey(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:ApiKey(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_API_KEY_ENTID JSON to run live")
      return
    end
    ...
    local api_key_ref01_list_result, err = api_key_ref01_ent:list(api_key_ref01_match, nil)
    assert.is_nil(err)
    ...
    local api_key_ref01_markdef_up0_name = "expires_at"
    local api_key_ref01_markdef_up0_value = "Mark01-api_key_ref01_" .. tostring(setup.now)
    api_key_ref01_data_up0_up[api_key_ref01_markdef_up0_name] = api_key_ref01_markdef_up0_value

    local api_key_ref01_resdata_up0_result, err = api_key_ref01_ent:update(api_key_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local api_key_ref01_resdata_up0 = helpers.to_map(api_key_ref01_resdata_up0_result)
    ...
    assert.is_nil(err)
    local api_key_ref01_data_dt0_load_result = helpers.to_map(api_key_ref01_data_dt0_loaded)
...
function api_key_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("BLUEFINPAYCONEX_TEST_API_KEY_ENTID")
