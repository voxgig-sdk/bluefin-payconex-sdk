...
describe("AccountUpdaterScheduleWithResultDirect", function()
  it("should direct-list-account_updater_schedule_with_result", function()
    local setup = account_updater_schedule_with_result_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-account_updater_schedule_with_result", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"account01", "schedule01"}) do
