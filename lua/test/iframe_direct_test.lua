...
describe("IframeDirect", function()
  it("should direct-list-iframe", function()
    local setup = iframe_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-iframe", setup.live and "live" or "unit")
  ...
  it("should direct-load-iframe", function()
    local setup = iframe_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-iframe", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function iframe_direct_setup(mockres)
