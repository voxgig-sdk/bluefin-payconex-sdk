...
defmodule BluefinPayconex.Entity.AccountUpdaterUpdate do
  alias Voxgig.Struct, as: S
  alias BluefinPayconex.Helpers, as: H
  alias BluefinPayconex.{EntityBase, Context, Pipeline}

  def new(client, entopts \\ nil) do
    EntityBase.construct(__MODULE__, client, "account_updater_update", entopts)
  end
  ...
  # Returns the account_updater_update entity map (BluefinPayconex.Types.account_updater_update/0) on
  # success; pipeline errors surface as the error value built by
  # Utility.make_error (shape is utility-configurable), hence term().
  @spec load(map(), BluefinPayconex.Types.account_updater_update_load_match() | nil, map() | nil) :: term()
