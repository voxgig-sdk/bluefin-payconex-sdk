...
defmodule BluefinPayconex.Entity.ThreeDSecureStatus do
  alias Voxgig.Struct, as: S
  alias BluefinPayconex.Helpers, as: H
  alias BluefinPayconex.{EntityBase, Context, Pipeline}

  def new(client, entopts \\ nil) do
    EntityBase.construct(__MODULE__, client, "three_d_secure_status", entopts)
  end
  ...
  # Returns the three_d_secure_status entity map (BluefinPayconex.Types.three_d_secure_status/0) on
  # success; pipeline errors surface as the error value built by
  # Utility.make_error (shape is utility-configurable), hence term().
  @spec load(map(), BluefinPayconex.Types.three_d_secure_status_load_match() | nil, map() | nil) :: term()
