...
defmodule BluefinPayconex.Entity.TransactionDetail do
  alias Voxgig.Struct, as: S
  alias BluefinPayconex.Helpers, as: H
  alias BluefinPayconex.{EntityBase, Context, Pipeline}

  def new(client, entopts \\ nil) do
    EntityBase.construct(__MODULE__, client, "transaction_detail", entopts)
  end
  ...
  # Returns the transaction_detail entity map (BluefinPayconex.Types.transaction_detail/0) on
  # success; pipeline errors surface as the error value built by
  # Utility.make_error (shape is utility-configurable), hence term().
  @spec load(map(), BluefinPayconex.Types.transaction_detail_load_match() | nil, map() | nil) :: term()
  ...
  # Returns the updated transaction_detail entity map (BluefinPayconex.Types.transaction_detail/0)
  # on success; pipeline errors surface as the error value built by
  # Utility.make_error (shape is utility-configurable), hence term().
  @spec update(map(), BluefinPayconex.Types.transaction_detail_update_data() | nil, map() | nil) :: term()
