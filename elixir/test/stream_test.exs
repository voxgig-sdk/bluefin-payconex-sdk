# BluefinPayconex SDK entity stream() test
#
...
defmodule BluefinPayconex.StreamTest do
  use ExUnit.Case
  ...
  defp has_feature?(name) do
    f = S.getprop(BluefinPayconex.Config.make_config(), "feature")
  ...
  defp entity(sdk, name), do: apply(BluefinPayconex, String.to_atom(name), [sdk, nil])
  defp entmod(name), do: Module.concat([BluefinPayconex.Entity, Macro.camelize(name)])
  ...
  test "entity stream() runs the pipeline and yields items" do
    ...
    if name == nil do
      ...
      sdk = BluefinPayconex.test(S.jm(["entity", seed(name)]), nil)
      items = do_stream(sdk, name, nil)
      assert length(items) == 3
      assert S.ismap(hd(items))

      # signal cancels iteration between yields.
      sdk2 = BluefinPayconex.test(S.jm(["entity", seed(name)]), nil)
