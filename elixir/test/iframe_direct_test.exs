# Iframe direct test (offline, mock system.fetch)

defmodule BluefinPayconex.IframeDirectTest do
  use ExUnit.Case
  ...
  test "should direct list iframe" do
    canned = S.jt([S.jm(["id", "direct01"]), S.jm(["id", "direct02"])])
    sdk = mk(canned)
    res = BluefinPayconex.direct(sdk, H.deep(%{"path" => "/iframe", "method" => "GET", "params" => %{}}))
  ...
  test "should direct load iframe" do
    canned = S.jm(["id", "direct01"])
    sdk = mk(canned)
    res = BluefinPayconex.direct(sdk, H.deep(%{"path" => "/iframe/direct01", "method" => "GET", "params" => %{}}))
    assert S.getprop(res, "ok") == true
    assert H.to_int(S.getprop(res, "status")) == 200
