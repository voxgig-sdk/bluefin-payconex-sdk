# DynamicDescriptor direct test (offline, mock system.fetch)

defmodule BluefinPayconex.DynamicDescriptorDirectTest do
  use ExUnit.Case
  ...
  test "should direct list dynamic_descriptor" do
    canned = S.jt([S.jm(["id", "direct01"]), S.jm(["id", "direct02"])])
    sdk = mk(canned)
    res = BluefinPayconex.direct(sdk, H.deep(%{"path" => "/dynamic_descriptor", "method" => "GET", "params" => %{}}))
  ...
  test "should direct load dynamic_descriptor" do
    canned = S.jm(["id", "direct01"])
    sdk = mk(canned)
    res = BluefinPayconex.direct(sdk, H.deep(%{"path" => "/dynamic_descriptor/direct01", "method" => "GET", "params" => %{}}))
    assert S.getprop(res, "ok") == true
    assert H.to_int(S.getprop(res, "status")) == 200
