...
# BluefinPayconex.FeatureHarness — the same hook order and short-circuit rules as
...
defmodule BluefinPayconex.FeatureTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinPayconex.Helpers, as: H
  alias BluefinPayconex.FeatureHarness, as: FH
    ...
    if has?("retry") do
      ...
      {server, calls} =
        FH.recording_server(fn n, _fd ->
          if n < 3,
            do: {nil, BluefinPayconex.Error.new("boom", "boom")},
  ...
  test "retry exhausted transport error surfaces" do
    if has?("retry") do
      clock = FH.make_clock()
      {server, calls} = FH.recording_server(fn _n, _fd -> {nil, BluefinPayconex.Error.new("boom", "boom")} end)
