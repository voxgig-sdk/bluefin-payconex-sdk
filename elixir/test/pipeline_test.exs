...
defmodule BluefinPayconex.PipelineFakeEntity do
...
defmodule BluefinPayconex.PipelineTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinPayconex.Helpers, as: H
  alias BluefinPayconex.{Utility, Context, Spec, Result, Response}

  defp client, do: BluefinPayconex.test()

  defp make_ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname, "ctrl", S.jm([])]), BluefinPayconex.get_root_ctx(client))
  end
  ...
  defp util_override(c, key, fun) do
    S.setprop(BluefinPayconex.get_utility(c), key, fun)
    c
  end
  ...
  defp resolve_order(feature) do
    ctx = Context.new(S.jm(["opname", "load", "ctrl", S.jm([])]), BluefinPayconex.get_root_ctx(client()))
  ...
  defp list_ctx(c, collector) do
    entity =
      S.jm(["_module", BluefinPayconex.PipelineFakeEntity, "_name", "widget", "_collector", collector])

    Context.new(S.jm(["opname", "list", "entity", entity, "ctrl", S.jm([])]), BluefinPayconex.get_root_ctx(c))
  end
  ...
  test "make_request a transport error tuple lands on the response" do
    boom = BluefinPayconex.Error.new("boom", "boom")
  ...
  test "make_request a fetchdef error surfaces as a response error" do
    c = util_override(client(), "make_fetch_def", fn _ctx -> {nil, BluefinPayconex.Error.new("fetchdef_boom", "boom")} end)
    ctx = make_ctx(c)
    S.setprop(ctx, "spec", request_spec())
    {response, err} = Utility.make_request(ctx)
    assert err == nil
  ...
  test "done raises the error when not ok" do
    ...
    assert_raise BluefinPayconex.Error, fn -> Utility.done(ctx) end
  end
  ...
  test "make_error preserves the error code" do
    ...
    err =
      assert_raise BluefinPayconex.Error, fn ->
  ...
  test "feature_add appends in call order" do
    ...
    a = BluefinPayconex.Feature.new()
    b = BluefinPayconex.Feature.new()
  ...
  defp auth_ctx(c, options, headers) do
    ...
    ctx =
      Context.new(
        S.jm(["opname", "load", "client", fakeclient, "utility", BluefinPayconex.get_utility(c), "ctrl", S.jm([])]),
        nil
      )
