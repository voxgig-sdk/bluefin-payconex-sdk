// Generated direct-call tests for the account_updater_schedule_with_result entity (mirrors the java
...
object AccountUpdaterScheduleWithResultDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-account_updater_schedule_with_result") {
      ...
      params.put("schedule_id", "direct02")
      val result = client.direct(SdkTestSupport.om(
          "path" -> "api/v4/accounts/{account_id}/account-updater/schedules/{schedule_id}/results",
          "method" -> "GET",
          "params" -> params))

      rep.eq("direct-list-account_updater_schedule_with_result.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-account_updater_schedule_with_result.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-account_updater_schedule_with_result.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-account_updater_schedule_with_result.size", 2, listData.size())
      rep.eqI("direct-list-account_updater_schedule_with_result.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-list-account_updater_schedule_with_result.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
      rep.check("direct-list-account_updater_schedule_with_result.url2", url.contains("direct02"), "expected url to contain direct02, got " + url)
    }
