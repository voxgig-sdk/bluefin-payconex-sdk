// Generated direct-call tests for the apple_pay_merchant_detail entity (mirrors the java
...
object ApplePayMerchantDetailDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-apple_pay_merchant_detail") {
      ...
      rep.eq("direct-list-apple_pay_merchant_detail.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-apple_pay_merchant_detail.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-apple_pay_merchant_detail.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-apple_pay_merchant_detail.size", 2, listData.size())
      rep.eqI("direct-list-apple_pay_merchant_detail.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-list-apple_pay_merchant_detail.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
