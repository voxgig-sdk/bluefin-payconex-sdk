// Generated direct-call tests for the three_d_secure_status entity (mirrors the java
...
object ThreeDSecureStatusDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-three_d_secure_status") {
      val setup = directSetup(SdkTestSupport.om("id" -> "direct01"))
      val client = setup.client

      val params = new LinkedHashMap[String, Object]()
      params.put("3_d_id", "direct01")
      params.put("account_id", "direct02")
      val result = client.direct(SdkTestSupport.om(
          "path" -> "api/v4/accounts/{account_id}/3DS/{3_d_id}/status",
          "method" -> "GET",
          "params" -> params))

      rep.eq("direct-load-three_d_secure_status.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-three_d_secure_status.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-three_d_secure_status.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-three_d_secure_status.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-three_d_secure_status.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-three_d_secure_status.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
      rep.check("direct-load-three_d_secure_status.url2", url.contains("direct02"), "expected url to contain direct02, got " + url)
    }
  }
}
