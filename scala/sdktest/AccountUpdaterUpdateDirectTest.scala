// Generated direct-call tests for the account_updater_update entity (mirrors the java
...
object AccountUpdaterUpdateDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-account_updater_update") {
      ...
      rep.eq("direct-load-account_updater_update.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-account_updater_update.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-account_updater_update.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-account_updater_update.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-account_updater_update.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-account_updater_update.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
      rep.check("direct-load-account_updater_update.url2", url.contains("direct02"), "expected url to contain direct02, got " + url)
    }
  }
}
