// Generated direct-call tests for the webhook entity (mirrors the java
...
object WebhookDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-webhook") {
      ...
      rep.eq("direct-list-webhook.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-webhook.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-webhook.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-webhook.size", 2, listData.size())
      rep.eqI("direct-list-webhook.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-list-webhook.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }

    rep.scope("direct-load-webhook") {
      ...
      rep.eq("direct-load-webhook.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-webhook.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-webhook.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-webhook.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-webhook.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-webhook.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
      rep.check("direct-load-webhook.url2", url.contains("direct02"), "expected url to contain direct02, got " + url)
    }
  }
}
