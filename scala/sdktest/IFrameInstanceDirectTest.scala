// Generated direct-call tests for the i_frame_instance entity (mirrors the java
...
object IFrameInstanceDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-i_frame_instance") {
      ...
      params.put("payment_iframe_id", "direct03")
      val result = client.direct(SdkTestSupport.om(
          "path" -> "api/v4/accounts/{account_id}/payment-iframe/{payment_iframe_id}/instance/{id}",
          "method" -> "GET",
          "params" -> params))

      rep.eq("direct-load-i_frame_instance.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-i_frame_instance.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-i_frame_instance.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-i_frame_instance.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-i_frame_instance.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-i_frame_instance.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
      rep.check("direct-load-i_frame_instance.url2", url.contains("direct02"), "expected url to contain direct02, got " + url)
      rep.check("direct-load-i_frame_instance.url3", url.contains("direct03"), "expected url to contain direct03, got " + url)
    }
  }
}
