// Generated direct-call tests for the iframe entity (mirrors the java
...
object IframeDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-iframe") {
      ...
      rep.eq("direct-list-iframe.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-iframe.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-iframe.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-iframe.size", 2, listData.size())
      rep.eqI("direct-list-iframe.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-list-iframe.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }

    rep.scope("direct-load-iframe") {
      val setup = directSetup(SdkTestSupport.om("id" -> "direct01"))
      val client = setup.client

      val params = new LinkedHashMap[String, Object]()
      params.put("account_id", "direct01")
      params.put("iframe_id", "direct02")
      val result = client.direct(SdkTestSupport.om(
          "path" -> "api/v4/accounts/{account_id}/payment-iframe/{iframe_id}",
          "method" -> "GET",
          "params" -> params))

      rep.eq("direct-load-iframe.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-iframe.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-iframe.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-iframe.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-iframe.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-iframe.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
      rep.check("direct-load-iframe.url2", url.contains("direct02"), "expected url to contain direct02, got " + url)
    }
  }
}
