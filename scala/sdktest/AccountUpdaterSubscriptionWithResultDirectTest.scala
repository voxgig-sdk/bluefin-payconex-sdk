// Generated direct-call tests for the account_updater_subscription_with_result entity (mirrors the java
...
object AccountUpdaterSubscriptionWithResultDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-account_updater_subscription_with_result") {
      ...
      params.put("subscription_id", "direct02")
      val result = client.direct(SdkTestSupport.om(
          "path" -> "api/v4/accounts/{account_id}/account-updater/subscriptions/{subscription_id}",
          "method" -> "GET",
          "params" -> params))

      rep.eq("direct-load-account_updater_subscription_with_result.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-account_updater_subscription_with_result.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-account_updater_subscription_with_result.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-account_updater_subscription_with_result.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-account_updater_subscription_with_result.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-account_updater_subscription_with_result.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
      rep.check("direct-load-account_updater_subscription_with_result.url2", url.contains("direct02"), "expected url to contain direct02, got " + url)
    }
  }
}
