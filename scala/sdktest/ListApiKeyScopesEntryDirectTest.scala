// Generated direct-call tests for the list_api_key_scopes_entry entity (mirrors the java
...
object ListApiKeyScopesEntryDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-list_api_key_scopes_entry") {
      ...
      rep.eq("direct-list-list_api_key_scopes_entry.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-list_api_key_scopes_entry.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-list_api_key_scopes_entry.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-list_api_key_scopes_entry.size", 2, listData.size())
      rep.eqI("direct-list-list_api_key_scopes_entry.calls", 1, setup.calls.size())
    }
