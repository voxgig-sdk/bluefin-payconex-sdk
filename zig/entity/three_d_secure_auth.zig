// ThreeDSecureAuth entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ThreeDSecureAuthEntity = struct {
    name: []const u8 = "three_d_secure_auth",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *ThreeDSecureAuthEntity {
        ...
        const e = h.A().create(ThreeDSecureAuthEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *ThreeDSecureAuthEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ThreeDSecureAuthEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ThreeDSecureAuthEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ThreeDSecureAuthEntity, ctx: *Context, post_done: *const fn (*ThreeDSecureAuthEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ThreeDSecureAuthEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ThreeDSecureAuthEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ThreeDSecureAuthEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ThreeDSecureAuthEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ThreeDSecureAuthEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ThreeDSecureAuthEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn create(self: *ThreeDSecureAuthEntity, reqdata: Value, ctrl: Value) OpResult {
    ...
    fn create_post_done(self: *ThreeDSecureAuthEntity, ctx: *Context) void {
