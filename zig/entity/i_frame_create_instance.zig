// IFrameCreateInstance entity client (generated — mirrors the go/rust Entity fragment).
...
pub const IFrameCreateInstanceEntity = struct {
    name: []const u8 = "i_frame_create_instance",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *IFrameCreateInstanceEntity {
        ...
        const e = h.A().create(IFrameCreateInstanceEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *IFrameCreateInstanceEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *IFrameCreateInstanceEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *IFrameCreateInstanceEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *IFrameCreateInstanceEntity, ctx: *Context, post_done: *const fn (*IFrameCreateInstanceEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *IFrameCreateInstanceEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *IFrameCreateInstanceEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return IFrameCreateInstanceEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *IFrameCreateInstanceEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *IFrameCreateInstanceEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *IFrameCreateInstanceEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn create(self: *IFrameCreateInstanceEntity, reqdata: Value, ctrl: Value) OpResult {
    ...
    fn create_post_done(self: *IFrameCreateInstanceEntity, ctx: *Context) void {
