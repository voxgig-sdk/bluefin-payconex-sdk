// AccountUpdater entity client (generated — mirrors the go/rust Entity fragment).
...
pub const AccountUpdaterEntity = struct {
    name: []const u8 = "account_updater",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *AccountUpdaterEntity {
        ...
        const e = h.A().create(AccountUpdaterEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *AccountUpdaterEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *AccountUpdaterEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *AccountUpdaterEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *AccountUpdaterEntity, ctx: *Context, post_done: *const fn (*AccountUpdaterEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *AccountUpdaterEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *AccountUpdaterEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return AccountUpdaterEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *AccountUpdaterEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *AccountUpdaterEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *AccountUpdaterEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn remove(self: *AccountUpdaterEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn remove_post_done(self: *AccountUpdaterEntity, ctx: *Context) void {
