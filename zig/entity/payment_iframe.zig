// PaymentIframe entity client (generated — mirrors the go/rust Entity fragment).
...
pub const PaymentIframeEntity = struct {
    name: []const u8 = "payment_iframe",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *PaymentIframeEntity {
        ...
        const e = h.A().create(PaymentIframeEntity) catch unreachable;
        e.* = .{
            .name = "payment_iframe",
            .client = client,
    ...
    fn ent_ctx(self: *PaymentIframeEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *PaymentIframeEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *PaymentIframeEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *PaymentIframeEntity, ctx: *Context, post_done: *const fn (*PaymentIframeEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *PaymentIframeEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *PaymentIframeEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return PaymentIframeEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *PaymentIframeEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *PaymentIframeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *PaymentIframeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn remove(self: *PaymentIframeEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn remove_post_done(self: *PaymentIframeEntity, ctx: *Context) void {
