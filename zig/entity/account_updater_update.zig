// AccountUpdaterUpdate entity client (generated — mirrors the go/rust Entity fragment).
...
pub const AccountUpdaterUpdateEntity = struct {
    name: []const u8 = "account_updater_update",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *AccountUpdaterUpdateEntity {
        ...
        const e = h.A().create(AccountUpdaterUpdateEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *AccountUpdaterUpdateEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *AccountUpdaterUpdateEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *AccountUpdaterUpdateEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *AccountUpdaterUpdateEntity, ctx: *Context, post_done: *const fn (*AccountUpdaterUpdateEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *AccountUpdaterUpdateEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *AccountUpdaterUpdateEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return AccountUpdaterUpdateEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *AccountUpdaterUpdateEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *AccountUpdaterUpdateEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *AccountUpdaterUpdateEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *AccountUpdaterUpdateEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *AccountUpdaterUpdateEntity, ctx: *Context) void {
