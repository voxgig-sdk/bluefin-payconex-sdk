// Webhook entity client (generated — mirrors the go/rust Entity fragment).
...
pub const WebhookEntity = struct {
    name: []const u8 = "webhook",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *WebhookEntity {
        ...
        const e = h.A().create(WebhookEntity) catch unreachable;
        e.* = .{
            .name = "webhook",
            .client = client,
    ...
    fn ent_ctx(self: *WebhookEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *WebhookEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *WebhookEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *WebhookEntity, ctx: *Context, post_done: *const fn (*WebhookEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *WebhookEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *WebhookEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return WebhookEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *WebhookEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *WebhookEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *WebhookEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *WebhookEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *WebhookEntity, ctx: *Context) void {
    ...
    pub fn list(self: *WebhookEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn list_post_done(self: *WebhookEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *WebhookEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *WebhookEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *WebhookEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn update_post_done(self: *WebhookEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
    ...
    pub fn remove(self: *WebhookEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *WebhookEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
