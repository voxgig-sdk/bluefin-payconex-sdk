// AccountUpdaterSchedule entity client (generated — mirrors the go/rust Entity fragment).
...
pub const AccountUpdaterScheduleEntity = struct {
    name: []const u8 = "account_updater_schedule",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *AccountUpdaterScheduleEntity {
        ...
        const e = h.A().create(AccountUpdaterScheduleEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *AccountUpdaterScheduleEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *AccountUpdaterScheduleEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *AccountUpdaterScheduleEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *AccountUpdaterScheduleEntity, ctx: *Context, post_done: *const fn (*AccountUpdaterScheduleEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *AccountUpdaterScheduleEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *AccountUpdaterScheduleEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return AccountUpdaterScheduleEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *AccountUpdaterScheduleEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *AccountUpdaterScheduleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *AccountUpdaterScheduleEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn create(self: *AccountUpdaterScheduleEntity, reqdata: Value, ctrl: Value) OpResult {
    ...
    fn create_post_done(self: *AccountUpdaterScheduleEntity, ctx: *Context) void {
