// ThreeDSecureStatus entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ThreeDSecureStatusEntity = struct {
    name: []const u8 = "three_d_secure_status",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *ThreeDSecureStatusEntity {
        ...
        const e = h.A().create(ThreeDSecureStatusEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *ThreeDSecureStatusEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ThreeDSecureStatusEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ThreeDSecureStatusEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ThreeDSecureStatusEntity, ctx: *Context, post_done: *const fn (*ThreeDSecureStatusEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ThreeDSecureStatusEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ThreeDSecureStatusEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ThreeDSecureStatusEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ThreeDSecureStatusEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ThreeDSecureStatusEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ThreeDSecureStatusEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ThreeDSecureStatusEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *ThreeDSecureStatusEntity, ctx: *Context) void {
