// ApplePayMerchantDetail entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ApplePayMerchantDetailEntity = struct {
    name: []const u8 = "apple_pay_merchant_detail",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *ApplePayMerchantDetailEntity {
        ...
        const e = h.A().create(ApplePayMerchantDetailEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *ApplePayMerchantDetailEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ApplePayMerchantDetailEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ApplePayMerchantDetailEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ApplePayMerchantDetailEntity, ctx: *Context, post_done: *const fn (*ApplePayMerchantDetailEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ApplePayMerchantDetailEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ApplePayMerchantDetailEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ApplePayMerchantDetailEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ApplePayMerchantDetailEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ApplePayMerchantDetailEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ApplePayMerchantDetailEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn list(self: *ApplePayMerchantDetailEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn list_post_done(self: *ApplePayMerchantDetailEntity, ctx: *Context) void {
    ...
    pub fn create(self: *ApplePayMerchantDetailEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *ApplePayMerchantDetailEntity, ctx: *Context) void {
        if (ctx.result) |result| {
