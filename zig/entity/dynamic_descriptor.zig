// DynamicDescriptor entity client (generated — mirrors the go/rust Entity fragment).
...
pub const DynamicDescriptorEntity = struct {
    name: []const u8 = "dynamic_descriptor",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *DynamicDescriptorEntity {
        ...
        const e = h.A().create(DynamicDescriptorEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *DynamicDescriptorEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DynamicDescriptorEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DynamicDescriptorEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *DynamicDescriptorEntity, ctx: *Context, post_done: *const fn (*DynamicDescriptorEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DynamicDescriptorEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DynamicDescriptorEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DynamicDescriptorEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DynamicDescriptorEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DynamicDescriptorEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DynamicDescriptorEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DynamicDescriptorEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *DynamicDescriptorEntity, ctx: *Context) void {
    ...
    pub fn list(self: *DynamicDescriptorEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn list_post_done(self: *DynamicDescriptorEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *DynamicDescriptorEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *DynamicDescriptorEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *DynamicDescriptorEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn update_post_done(self: *DynamicDescriptorEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
    ...
    pub fn remove(self: *DynamicDescriptorEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *DynamicDescriptorEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
