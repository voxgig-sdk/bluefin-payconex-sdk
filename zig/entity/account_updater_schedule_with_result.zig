// AccountUpdaterScheduleWithResult entity client (generated — mirrors the go/rust Entity fragment).
...
pub const AccountUpdaterScheduleWithResultEntity = struct {
    name: []const u8 = "account_updater_schedule_with_result",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *AccountUpdaterScheduleWithResultEntity {
        ...
        const e = h.A().create(AccountUpdaterScheduleWithResultEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *AccountUpdaterScheduleWithResultEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *AccountUpdaterScheduleWithResultEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *AccountUpdaterScheduleWithResultEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *AccountUpdaterScheduleWithResultEntity, ctx: *Context, post_done: *const fn (*AccountUpdaterScheduleWithResultEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *AccountUpdaterScheduleWithResultEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *AccountUpdaterScheduleWithResultEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return AccountUpdaterScheduleWithResultEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *AccountUpdaterScheduleWithResultEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *AccountUpdaterScheduleWithResultEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *AccountUpdaterScheduleWithResultEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn list(self: *AccountUpdaterScheduleWithResultEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn list_post_done(self: *AccountUpdaterScheduleWithResultEntity, ctx: *Context) void {
    ...
    pub fn create(self: *AccountUpdaterScheduleWithResultEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *AccountUpdaterScheduleWithResultEntity, ctx: *Context) void {
        if (ctx.result) |result| {
