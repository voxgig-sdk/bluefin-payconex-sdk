// ThreeDSecureBrowserInit entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ThreeDSecureBrowserInitEntity = struct {
    name: []const u8 = "three_d_secure_browser_init",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *ThreeDSecureBrowserInitEntity {
        ...
        const e = h.A().create(ThreeDSecureBrowserInitEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *ThreeDSecureBrowserInitEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ThreeDSecureBrowserInitEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ThreeDSecureBrowserInitEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ThreeDSecureBrowserInitEntity, ctx: *Context, post_done: *const fn (*ThreeDSecureBrowserInitEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ThreeDSecureBrowserInitEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ThreeDSecureBrowserInitEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ThreeDSecureBrowserInitEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ThreeDSecureBrowserInitEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ThreeDSecureBrowserInitEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ThreeDSecureBrowserInitEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn create(self: *ThreeDSecureBrowserInitEntity, reqdata: Value, ctrl: Value) OpResult {
    ...
    fn create_post_done(self: *ThreeDSecureBrowserInitEntity, ctx: *Context) void {
