// ListApiKeyScopesEntry entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ListApiKeyScopesEntryEntity = struct {
    name: []const u8 = "list_api_key_scopes_entry",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *ListApiKeyScopesEntryEntity {
        ...
        const e = h.A().create(ListApiKeyScopesEntryEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *ListApiKeyScopesEntryEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ListApiKeyScopesEntryEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ListApiKeyScopesEntryEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ListApiKeyScopesEntryEntity, ctx: *Context, post_done: *const fn (*ListApiKeyScopesEntryEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ListApiKeyScopesEntryEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ListApiKeyScopesEntryEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ListApiKeyScopesEntryEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ListApiKeyScopesEntryEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ListApiKeyScopesEntryEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ListApiKeyScopesEntryEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn list(self: *ListApiKeyScopesEntryEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn list_post_done(self: *ListApiKeyScopesEntryEntity, ctx: *Context) void {
