// ApiKey entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ApiKeyEntity = struct {
    name: []const u8 = "api_key",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *ApiKeyEntity {
        ...
        const e = h.A().create(ApiKeyEntity) catch unreachable;
        e.* = .{
            .name = "api_key",
            .client = client,
    ...
    fn ent_ctx(self: *ApiKeyEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ApiKeyEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ApiKeyEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ApiKeyEntity, ctx: *Context, post_done: *const fn (*ApiKeyEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ApiKeyEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ApiKeyEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ApiKeyEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ApiKeyEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ApiKeyEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ApiKeyEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *ApiKeyEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *ApiKeyEntity, ctx: *Context) void {
    ...
    pub fn list(self: *ApiKeyEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "list",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn list_post_done(self: *ApiKeyEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            if (resmatch == .object) self.mtch = resmatch;
        }
    ...
    pub fn create(self: *ApiKeyEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *ApiKeyEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resdata = result.resdata;
            if (!h.is_noval(resdata)) {
                const cm = h.to_map(h.clone(resdata));
    ...
    pub fn update(self: *ApiKeyEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn update_post_done(self: *ApiKeyEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
    ...
    pub fn remove(self: *ApiKeyEntity, reqmatch: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "remove",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn remove_post_done(self: *ApiKeyEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
