// ApplePaySession entity client (generated — mirrors the go/rust Entity fragment).
...
pub const ApplePaySessionEntity = struct {
    name: []const u8 = "apple_pay_session",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *ApplePaySessionEntity {
        ...
        const e = h.A().create(ApplePaySessionEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *ApplePaySessionEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *ApplePaySessionEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *ApplePaySessionEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *ApplePaySessionEntity, ctx: *Context, post_done: *const fn (*ApplePaySessionEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *ApplePaySessionEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *ApplePaySessionEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return ApplePaySessionEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *ApplePaySessionEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *ApplePaySessionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *ApplePaySessionEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn create(self: *ApplePaySessionEntity, reqdata: Value, ctrl: Value) OpResult {
    ...
    fn create_post_done(self: *ApplePaySessionEntity, ctx: *Context) void {
