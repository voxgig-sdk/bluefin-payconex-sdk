// TransactionDetail entity client (generated — mirrors the go/rust Entity fragment).
...
pub const TransactionDetailEntity = struct {
    name: []const u8 = "transaction_detail",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *TransactionDetailEntity {
        ...
        const e = h.A().create(TransactionDetailEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *TransactionDetailEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *TransactionDetailEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *TransactionDetailEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *TransactionDetailEntity, ctx: *Context, post_done: *const fn (*TransactionDetailEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *TransactionDetailEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *TransactionDetailEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return TransactionDetailEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *TransactionDetailEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *TransactionDetailEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *TransactionDetailEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *TransactionDetailEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *TransactionDetailEntity, ctx: *Context) void {
    ...
    pub fn update(self: *TransactionDetailEntity, reqdata: Value, ctrl: Value) OpResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "update",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn update_post_done(self: *TransactionDetailEntity, ctx: *Context) void {
        if (ctx.result) |result| {
            const resmatch = result.resmatch;
            const resdata = result.resdata;
            if (resmatch == .object) self.mtch = resmatch;
