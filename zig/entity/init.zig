// Init entity client (generated — mirrors the go/rust Entity fragment).
...
pub const InitEntity = struct {
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *InitEntity {
        ...
        const e = h.A().create(InitEntity) catch unreachable;
        e.* = .{
            .name = "init",
            .client = client,
    ...
    fn ent_ctx(self: *InitEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *InitEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *InitEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *InitEntity, ctx: *Context, post_done: *const fn (*InitEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *InitEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *InitEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return InitEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *InitEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *InitEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *InitEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn create(self: *InitEntity, reqdata: Value, ctrl: Value) OpResult {
    ...
    fn create_post_done(self: *InitEntity, ctx: *Context) void {
