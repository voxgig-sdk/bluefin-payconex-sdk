// IFrameInstance entity client (generated — mirrors the go/rust Entity fragment).
...
pub const IFrameInstanceEntity = struct {
    name: []const u8 = "i_frame_instance",
    ...
    pub fn new(client: *sdk.BluefinPayconexSDK, entopts_in: Value) *IFrameInstanceEntity {
        ...
        const e = h.A().create(IFrameInstanceEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *IFrameInstanceEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *IFrameInstanceEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *IFrameInstanceEntity, ctx: *Context) OpResult {
        const v = self.utility.done(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn run_op(self: *IFrameInstanceEntity, ctx: *Context, post_done: *const fn (*IFrameInstanceEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *IFrameInstanceEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *IFrameInstanceEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return IFrameInstanceEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *IFrameInstanceEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *IFrameInstanceEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *IFrameInstanceEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *IFrameInstanceEntity, reqmatch: Value, ctrl: Value) OpResult {
    ...
    fn load_post_done(self: *IFrameInstanceEntity, ctx: *Context) void {
