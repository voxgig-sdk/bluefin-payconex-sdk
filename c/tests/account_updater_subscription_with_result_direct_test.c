// Generated direct-call test for the account_updater_subscription_with_result entity (mirrors the
...
static voxgig_value* account_updater_subscription_with_result_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinPayconexSDK* account_updater_subscription_with_result_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(account_updater_subscription_with_result_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinPayconexSDK* sdk = account_updater_subscription_with_result_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "account_id", v_str("direct01"));
    setp(params, "subscription_id", v_str("direct02"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("api/v4/accounts/{account_id}/account-updater/subscriptions/{subscription_id}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_payconex_account_updater_subscription_with_result_direct");
}
