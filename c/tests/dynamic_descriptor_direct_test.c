// Generated direct-call test for the dynamic_descriptor entity (mirrors the
...
static voxgig_value* dynamic_descriptor_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinPayconexSDK* dynamic_descriptor_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(dynamic_descriptor_mock, mockres)));
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinPayconexSDK* sdk = dynamic_descriptor_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "account_id", v_str("direct01"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("api/v4/accounts/{account_id}/dynamic_descriptors"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  // LOAD
  {
    ...
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("api/v4/accounts/{account_id}/dynamic_descriptors/{id}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_payconex_dynamic_descriptor_direct");
}
