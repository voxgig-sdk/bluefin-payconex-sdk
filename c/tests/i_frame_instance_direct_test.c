// Generated direct-call test for the i_frame_instance entity (mirrors the
...
static voxgig_value* i_frame_instance_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinPayconexSDK* i_frame_instance_direct_setup(voxgig_value* mockres) {
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinPayconexSDK* sdk = i_frame_instance_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "account_id", v_str("direct01"));
    setp(params, "id", v_str("direct02"));
    setp(params, "payment_iframe_id", v_str("direct03"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("api/v4/accounts/{account_id}/payment-iframe/{payment_iframe_id}/instance/{id}"),
      "method", v_str("GET"),
      "params", params), &err);
