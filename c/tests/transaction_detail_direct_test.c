// Generated direct-call test for the transaction_detail entity (mirrors the
...
static voxgig_value* transaction_detail_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinPayconexSDK* transaction_detail_direct_setup(voxgig_value* mockres) {
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinPayconexSDK* sdk = transaction_detail_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "account_id", v_str("direct01"));
    setp(params, "transaction_id", v_str("direct02"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("api/v4/accounts/{account_id}/payments/{transaction_id}"),
      "method", v_str("GET"),
      "params", params), &err);
