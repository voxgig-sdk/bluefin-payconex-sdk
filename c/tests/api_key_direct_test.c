// Generated direct-call test for the api_key entity (mirrors the
...
static voxgig_value* api_key_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinPayconexSDK* api_key_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(api_key_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("api/v4/accounts/{account_id}/api-keys/{id}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_payconex_api_key_direct");
}
