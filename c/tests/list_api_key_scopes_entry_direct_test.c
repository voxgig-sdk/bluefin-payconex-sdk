// Generated direct-call test for the list_api_key_scopes_entry entity (mirrors the
...
static voxgig_value* list_api_key_scopes_entry_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinPayconexSDK* list_api_key_scopes_entry_direct_setup(voxgig_value* mockres) {
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinPayconexSDK* sdk = list_api_key_scopes_entry_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("api/v4/api-key-scopes"),
      "method", v_str("GET"),
      "params", params), &err);
