// Generated direct-call test for the iframe entity (mirrors the
...
static voxgig_value* iframe_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinPayconexSDK* iframe_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(iframe_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    voxgig_value* params = v_map();
    setp(params, "account_id", v_str("direct01"));
    setp(params, "iframe_id", v_str("direct02"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("api/v4/accounts/{account_id}/payment-iframe/{iframe_id}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_payconex_iframe_direct");
}
