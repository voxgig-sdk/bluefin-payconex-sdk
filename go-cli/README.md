# bluefin-payconex-cli

AQL-driven command-line client **and** interactive REPL for the BluefinPayconex
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-payconex-cli)
make build

# 2. See usage (words, entities, env vars)
./bluefin-payconex-cli --help
...
BLUEFIN_PAYCONEX_BASE=https://api.example.com ./bluefin-payconex-cli --help

# 6. No arguments -> interactive REPL
./bluefin-payconex-cli
bluefin-payconex> /help
bluefin-payconex> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-payconex-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bluefin-payconex-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `bluefin-payconex>`). Each line is
...
below — this SDK exposes 20 entities.
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `account_updater`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bluefin-payconex-cli`. |
...
The 20 entities this SDK exposes (any is valid as `<entity>`):
...
(`./bluefin-payconex-cli <expr>`) and interactively (the REPL), and expressions compose the
