// Generated direct-call tests for the transaction_detail entity (unit mode;
...
static TransactionDetailDirectSetup transaction_detail_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  TransactionDetailDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void transaction_detail_direct_load() {
  auto setup = transaction_detail_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-transaction_detail", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "account_id", Value("direct01"));
  setp(params, "transaction_id", Value("direct02"));

  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/payments/{transaction_id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(transaction_detail_direct_load);
  return sdktest::summary("transaction_detail_direct_test");
}
