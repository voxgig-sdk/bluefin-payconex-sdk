// Generated direct-call tests for the apple_pay_merchant_detail entity (unit mode;
...
static ApplePayMerchantDetailDirectSetup apple_pay_merchant_detail_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  ApplePayMerchantDetailDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void apple_pay_merchant_detail_direct_list() {
  auto setup = apple_pay_merchant_detail_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-apple_pay_merchant_detail", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/applePay/enrollment")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(apple_pay_merchant_detail_direct_list);
  return sdktest::summary("apple_pay_merchant_detail_direct_test");
}
