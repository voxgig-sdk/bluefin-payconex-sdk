// BluefinPayconex SDK — shared test runner support (mirrors java
...
inline Value env_override(Value m) {
  std::string live = getenv_local("BLUEFINPAYCONEX_TEST_LIVE");
  std::string over = getenv_local("BLUEFINPAYCONEX_TEST_OVERRIDE");
  ...
  std::string explain = getenv_local("BLUEFINPAYCONEX_TEST_EXPLAIN");
  if (!explain.empty()) map_put(m, "BLUEFINPAYCONEX_TEST_EXPLAIN", Value(explain));
  return m;
}
...
inline CtxPtr make_ctx_from_map(const Value& ctxmap_, std::shared_ptr<BluefinPayconexSDK> client,
                                UtilityPtr utility) {
...
inline void fixctx(CtxPtr ctx, std::shared_ptr<BluefinPayconexSDK> client) {
