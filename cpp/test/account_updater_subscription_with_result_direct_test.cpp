// Generated direct-call tests for the account_updater_subscription_with_result entity (unit mode;
...
static AccountUpdaterSubscriptionWithResultDirectSetup account_updater_subscription_with_result_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  AccountUpdaterSubscriptionWithResultDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void account_updater_subscription_with_result_direct_load() {
  auto setup = account_updater_subscription_with_result_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-account_updater_subscription_with_result", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "account_id", Value("direct01"));
  setp(params, "subscription_id", Value("direct02"));

  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/account-updater/subscriptions/{subscription_id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(account_updater_subscription_with_result_direct_load);
  return sdktest::summary("account_updater_subscription_with_result_direct_test");
}
