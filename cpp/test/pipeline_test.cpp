// BluefinPayconex SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinPayconexSDK> plClient(const Value& sdkopts) {
  return BluefinPayconexSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinPayconexSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinPayconexSDK> client) {
  std::string out;
