// Generated direct-call tests for the api_key entity (unit mode;
...
static ApiKeyDirectSetup api_key_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void api_key_direct_list() {
  auto setup = api_key_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-api_key", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/api-keys")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
static void api_key_direct_load() {
  auto setup = api_key_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-api_key", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/api-keys/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(api_key_direct_list);
  T_RUN(api_key_direct_load);
  return sdktest::summary("api_key_direct_test");
}
