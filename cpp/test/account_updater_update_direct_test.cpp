// Generated direct-call tests for the account_updater_update entity (unit mode;
...
static AccountUpdaterUpdateDirectSetup account_updater_update_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  AccountUpdaterUpdateDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void account_updater_update_direct_load() {
  auto setup = account_updater_update_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-account_updater_update", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/account-updater/updates/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(account_updater_update_direct_load);
  return sdktest::summary("account_updater_update_direct_test");
}
