// Generated direct-call tests for the iframe entity (unit mode;
...
static IframeDirectSetup iframe_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void iframe_direct_list() {
  auto setup = iframe_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-iframe", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/payment-iframe")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
static void iframe_direct_load() {
  auto setup = iframe_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-iframe", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "account_id", Value("direct01"));
  setp(params, "iframe_id", Value("direct02"));

  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/payment-iframe/{iframe_id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(iframe_direct_list);
  T_RUN(iframe_direct_load);
  return sdktest::summary("iframe_direct_test");
}
