// Generated direct-call tests for the webhook entity (unit mode;
...
static WebhookDirectSetup webhook_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void webhook_direct_list() {
  auto setup = webhook_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-webhook", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/webhooks")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
static void webhook_direct_load() {
  auto setup = webhook_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-webhook", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/webhooks/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(webhook_direct_list);
  T_RUN(webhook_direct_load);
  return sdktest::summary("webhook_direct_test");
}
