// Generated direct-call tests for the i_frame_instance entity (unit mode;
...
static IFrameInstanceDirectSetup i_frame_instance_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void i_frame_instance_direct_load() {
  auto setup = i_frame_instance_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-i_frame_instance", "unit");
  ...
  setp(params, "payment_iframe_id", Value("direct03"));

  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/payment-iframe/{payment_iframe_id}/instance/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(i_frame_instance_direct_load);
  return sdktest::summary("i_frame_instance_direct_test");
}
