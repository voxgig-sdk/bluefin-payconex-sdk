// Generated direct-call tests for the dynamic_descriptor entity (unit mode;
...
static DynamicDescriptorDirectSetup dynamic_descriptor_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  DynamicDescriptorDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void dynamic_descriptor_direct_list() {
  auto setup = dynamic_descriptor_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-dynamic_descriptor", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/dynamic_descriptors")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
static void dynamic_descriptor_direct_load() {
  auto setup = dynamic_descriptor_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-dynamic_descriptor", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/dynamic_descriptors/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(dynamic_descriptor_direct_list);
  T_RUN(dynamic_descriptor_direct_load);
  return sdktest::summary("dynamic_descriptor_direct_test");
}
