// Generated direct-call tests for the three_d_secure_status entity (unit mode;
...
static ThreeDSecureStatusDirectSetup three_d_secure_status_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  ThreeDSecureStatusDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void three_d_secure_status_direct_load() {
  auto setup = three_d_secure_status_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-three_d_secure_status", "unit");
  if (sk.first) { std::cerr << "skip\n"; return; }
  auto client = setup.client;

  Value params = vmap();
  setp(params, "3_d_id", Value("direct01"));
  setp(params, "account_id", Value("direct02"));

  Value result = client->direct(vmap({
    {"path", Value("api/v4/accounts/{account_id}/3DS/{3_d_id}/status")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(three_d_secure_status_direct_load);
  return sdktest::summary("three_d_secure_status_direct_test");
}
