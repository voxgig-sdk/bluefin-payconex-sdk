// Generated direct-call tests for the list_api_key_scopes_entry entity (unit mode;
...
static ListApiKeyScopesEntryDirectSetup list_api_key_scopes_entry_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  ListApiKeyScopesEntryDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void list_api_key_scopes_entry_direct_list() {
  auto setup = list_api_key_scopes_entry_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-list_api_key_scopes_entry", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("api/v4/api-key-scopes")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(list_api_key_scopes_entry_direct_list);
  return sdktest::summary("list_api_key_scopes_entry_direct_test");
}
