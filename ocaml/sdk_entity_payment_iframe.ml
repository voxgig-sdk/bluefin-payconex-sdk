...
let rec make (client : sdk_client) (entopts_in : value) : entity_obj =
  ...
  ent.e_load <- (fun _ _ ->
      raise (Sdk_error_exc (mk_error "unsupported_op"
        "Operation \"load\" not supported by entity \"payment_iframe\".")));
  ent.e_list <- (fun _ _ ->
      raise (Sdk_error_exc (mk_error "unsupported_op"
        "Operation \"list\" not supported by entity \"payment_iframe\".")));
  ent.e_create <- (fun _ _ ->
      raise (Sdk_error_exc (mk_error "unsupported_op"
        "Operation \"create\" not supported by entity \"payment_iframe\".")));
  ent.e_update <- (fun _ _ ->
      raise (Sdk_error_exc (mk_error "unsupported_op"
        "Operation \"update\" not supported by entity \"payment_iframe\".")));
