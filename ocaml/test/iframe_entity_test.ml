...
let () =
  test "iframe.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.iframe client Noval in
      check_str "name" ent.e_name "iframe")

let () =
  test "iframe.seeded_ops" (fun () ->
      let record = jo [("id", Str "iframe01")] in
      let seed = jo [("iframe",
                      jo [("iframe01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "iframe01")]) Noval in
      check "load is a map" (ismap loaded);
      check_vstr "load id" (getp loaded "id") "iframe01";
      ())

let () =
  test "iframe.stream" (fun () ->
      let mk_seed () =
        jo [("iframe",
      ...
      let ent2 = Sdk_client.iframe client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.iframe sclient Noval in
        ...
        let cent = Sdk_client.iframe cclient Noval in
