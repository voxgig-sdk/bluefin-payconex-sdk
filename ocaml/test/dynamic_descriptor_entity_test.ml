(* Generated dynamic_descriptor entity test. *)
...
let () =
  test "dynamic_descriptor.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.dynamic_descriptor client Noval in
      check_str "name" ent.e_name "dynamic_descriptor")

let () =
  test "dynamic_descriptor.seeded_ops" (fun () ->
      let record = jo [("id", Str "dynamic_descriptor01")] in
      let seed = jo [("dynamic_descriptor",
                      jo [("dynamic_descriptor01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "dynamic_descriptor01")]) Noval in
      check "load is a map" (ismap loaded);
      check_vstr "load id" (getp loaded "id") "dynamic_descriptor01";
      ())

let () =
  test "dynamic_descriptor.stream" (fun () ->
      let mk_seed () =
        jo [("dynamic_descriptor",
      ...
      let ent2 = Sdk_client.dynamic_descriptor client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.dynamic_descriptor sclient Noval in
        ...
        let cent = Sdk_client.dynamic_descriptor cclient Noval in
