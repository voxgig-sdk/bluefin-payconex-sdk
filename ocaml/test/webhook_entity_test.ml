...
let () =
  test "webhook.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.webhook client Noval in
      check_str "name" ent.e_name "webhook")

let () =
  test "webhook.seeded_ops" (fun () ->
      let record = jo [("id", Str "webhook01")] in
      let seed = jo [("webhook",
                      jo [("webhook01", record)])] in
      ...
      let loaded = ent.e_load (jo [("id", Str "webhook01")]) Noval in
      check "load is a map" (ismap loaded);
      check_vstr "load id" (getp loaded "id") "webhook01";
      ())

let () =
  test "webhook.stream" (fun () ->
      let mk_seed () =
        jo [("webhook",
      ...
      let ent2 = Sdk_client.webhook client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.webhook sclient Noval in
        ...
        let cent = Sdk_client.webhook cclient Noval in
