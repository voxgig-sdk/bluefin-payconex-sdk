...
fn default_http_fetch(fullurl: &str, fetchdef: &Value) -> Result<Value, BluefinPayconexError> {
    ...
    if !has_ua {
        req = req.set("User-Agent", "Mozilla/5.0 (compatible; BluefinPayconexSDK/1.0)");
    }
    ...
    let resp = match sent {
        ...
        Err(e) => {
            return Err(BluefinPayconexError::new("fetch_transport", &format!("{}", e)));
        }
    };
    ...
    let body_txt = resp
        .into_string()
        .map_err(|e| BluefinPayconexError::new("fetch_body", &format!("{}", e)))?;
