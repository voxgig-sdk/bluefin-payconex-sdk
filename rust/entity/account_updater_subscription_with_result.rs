// AccountUpdaterSubscriptionWithResult entity client (generated — mirrors the go Entity fragment).
...
use crate::core::sdk::BluefinPayconexSDK;
use crate::core::types::{Entity, BluefinPayconexEntity};
use crate::core::utility_type::Utility;
use crate::utility::voxgigstruct as vs;
use crate::utility::voxgigstruct::Value;

pub struct AccountUpdaterSubscriptionWithResultEntity {
    name: String,
    #[allow(dead_code)]
...
impl AccountUpdaterSubscriptionWithResultEntity {
    pub fn new(client: &Rc<BluefinPayconexSDK>, entopts: Value) -> Rc<AccountUpdaterSubscriptionWithResultEntity> {
        ...
        let e = Rc::new(AccountUpdaterSubscriptionWithResultEntity {
            name: "account_updater_subscription_with_result".to_string(),
            client: client.clone(),
    ...
    ) -> Result<Value, BluefinPayconexError> {
    ...
    pub fn stream(
        ...
        args: Value,
        callopts: Value,
    ) -> Result<std::vec::IntoIter<Value>, BluefinPayconexError> {
...
impl Entity for AccountUpdaterSubscriptionWithResultEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        AccountUpdaterSubscriptionWithResultEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinPayconexEntity for AccountUpdaterSubscriptionWithResultEntity {

    fn load(&self, reqmatch: Value, ctrl: Value) -> Result<Value, BluefinPayconexError> {
        let ctx = self.utility.make_context(
            CtxSpec {
    ...
    fn list(&self, _reqmatch: Value, _ctrl: Value) -> Result<Value, BluefinPayconexError> {
        Err(crate::core::helpers::unsupported_op("list", &self.name))
    }


    fn create(&self, reqdata: Value, ctrl: Value) -> Result<Value, BluefinPayconexError> {
        let ctx = self.utility.make_context(
            CtxSpec {
    ...
    fn update(&self, reqdata: Value, ctrl: Value) -> Result<Value, BluefinPayconexError> {
        let ctx = self.utility.make_context(
            CtxSpec {
    ...
    fn remove(&self, _reqmatch: Value, _ctrl: Value) -> Result<Value, BluefinPayconexError> {
