...
/// env_override (mirrors go): when BLUEFINPAYCONEX_TEST_LIVE or
/// BLUEFINPAYCONEX_TEST_OVERRIDE is TRUE, environment variables replace the
/// defaults in `m` (JSON-shaped values are parsed).
pub fn env_override(m: Value) -> Value {
    let live = std::env::var("BLUEFINPAYCONEX_TEST_LIVE").unwrap_or_default() == "TRUE";
    let over = std::env::var("BLUEFINPAYCONEX_TEST_OVERRIDE").unwrap_or_default() == "TRUE";
    ...
    if let Ok(explain) = std::env::var("BLUEFINPAYCONEX_TEST_EXPLAIN") {
        if !explain.is_empty() {
            setp(&m, "BLUEFINPAYCONEX_TEST_EXPLAIN", Value::str(explain));
        }
    }
...
pub struct EntityTestSetup {
    pub client: Rc<BluefinPayconexSDK>,
    pub data: Value,
    pub idmap: Value,
    pub env: Value,
    pub explain: bool,
...
pub fn match_string(pattern: &str, val: &str) -> bool {
    if pattern.len() >= 2 && pattern.starts_with('/') && pattern.ends_with('/') {
        return bluefin_payconex_sdk::utility::voxgigstruct::re::Regex::new(
...
pub fn runset(
    testspec: &Value,
    subject: &mut dyn FnMut(&Value) -> Result<Value, BluefinPayconexError>,
) {
    ...
    if let Value::Map(_) = getp(&ctxmap, "result") {
        ...
        if let Value::Map(_) = getp(&res_map, "err") {
            if let Some(msg) = get_str(&getp(&res_map, "err"), "message") {
                result.borrow_mut().err = Some(BluefinPayconexError::new("", &msg));
            }
        }
...
pub fn fixctx(ctx: &Rc<Context>, client: &Rc<BluefinPayconexSDK>) {
    if ctx.options.borrow().is_noval() {
        *ctx.options.borrow_mut() = client.options_map();
    }
}
...
pub fn err_from_map(m: &Value) -> Option<BluefinPayconexError> {
    let msg = get_str(m, "message").unwrap_or_default();
    if msg.is_empty() {
        return None;
    }
    let code = get_str(m, "code").unwrap_or_default();
    Some(BluefinPayconexError::new(&code, &msg))
}
...
pub fn fh_has_feature(name: &str) -> bool {
    let config = bluefin_payconex_sdk::make_config();
...
pub type FhReply = Rc<dyn Fn(i64, &Value) -> Result<Value, BluefinPayconexError>>;
...
pub struct FhOpResult {
    pub ok: bool,
    pub data: Value,
    pub err: Option<BluefinPayconexError>,
...
fn fh_populate_result(ctx: &Rc<Context>, fetched: &Result<Value, BluefinPayconexError>) {
    ...
    *ctx.result.borrow_mut() = Some(result.clone());
    ...
    let resp = Response::new(response_map);
    {
        ...
        if let Value::Func(_) = &resp.json {
            r.body = bluefin_payconex_sdk::call_json(&resp.json);
        }
...
impl FhHarness {
    ...
    pub fn op(&self, o: FhOpSpec) -> FhOpResult {
        ...
        *ctx.spec.borrow_mut() = Some(spec.clone());
        ...
        let fetched: Result<Value, BluefinPayconexError> = match ctx.out_get("request") {
    ...
    fn fail(&self, ctx: Rc<Context>, err: BluefinPayconexError) -> FhOpResult {
        {
...
pub fn fh_err_code(err: &Option<BluefinPayconexError>) -> String {
