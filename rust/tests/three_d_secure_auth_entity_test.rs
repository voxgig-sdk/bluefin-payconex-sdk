...
#[test]
fn three_d_secure_auth_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.three_d_secure_auth(Value::Noval);
    assert_eq!(ent.get_name(), "three_d_secure_auth");
}

#[test]
fn three_d_secure_auth_entity_stream() {
    ...
    let plainent = plainsdk.three_d_secure_auth(Value::Noval);
...
#[test]
fn three_d_secure_auth_entity_basic() {
    let setup = three_d_secure_auth_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("three_d_secure_auth.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let three_d_secure_auth_ref01_ent = client.three_d_secure_auth(Value::Noval);
    let three_d_secure_auth_ref01_data = to_map(&getp(
        &getpath(&["new", "three_d_secure_auth"], &setup.data),
    ...
    let three_d_secure_auth_ref01_data_result = three_d_secure_auth_ref01_ent
        .create(three_d_secure_auth_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let three_d_secure_auth_ref01_data = to_map(&three_d_secure_auth_ref01_data_result);
    assert!(
        matches!(three_d_secure_auth_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn three_d_secure_auth_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("three_d_secure_auth");
    entity_data_file.push("ThreeDSecureAuthTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("three_d_secure_auth01"), Value::str("three_d_secure_auth02"), Value::str("three_d_secure_auth03"), Value::str("account01"), Value::str("account02"), Value::str("account03"), Value::str("3_d01"), Value::str("3_d02"), Value::str("3_d03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINPAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID")) {
