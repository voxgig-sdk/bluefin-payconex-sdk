// Generated direct-call tests for the three_d_secure_status entity (mirrors the
...
struct ThreeDSecureStatusDirectSetup {
...
fn three_d_secure_status_direct_setup(mockres: Value) -> ThreeDSecureStatusDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_THREE_D_SECURE_STATUS_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_THREE_D_SECURE_STATUS_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return ThreeDSecureStatusDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn three_d_secure_status_direct_load() {
    let setup = three_d_secure_status_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-three_d_secure_status", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["3_d01", "account01"] {
    ...
    if setup.live {
        setp(&params, "3_d_id", getp(&setup.idmap, "3_d01"));
    } else {
        setp(&params, "3_d_id", Value::str("direct01"));
    }
    if setup.live {
        setp(&params, "account_id", getp(&setup.idmap, "account01"));
    } else {
        setp(&params, "account_id", Value::str("direct02"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/3DS/{3_d_id}/status")),
