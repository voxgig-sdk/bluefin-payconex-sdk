// Generated direct-call tests for the iframe entity (mirrors the
...
fn iframe_direct_setup(mockres: Value) -> IframeDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_IFRAME_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_IFRAME_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return IframeDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn iframe_direct_list() {
    let setup = iframe_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-iframe", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/payment-iframe")),
...
#[test]
fn iframe_direct_load() {
    let setup = iframe_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-iframe", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["account01", "iframe01"] {
            if getp(&setup.idmap, live_key).is_noval() {
                eprintln!("skip: live test needs {} via *_ENTID env var (synthetic IDs only)", live_key);
                return;
            }
    ...
    if setup.live {
        setp(&params, "iframe_id", getp(&setup.idmap, "iframe01"));
    } else {
        setp(&params, "iframe_id", Value::str("direct02"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/payment-iframe/{iframe_id}")),
            ("method", Value::str("GET")),
            ("params", params.clone()),
        ]))
        .expect("direct failed");
