// Generated direct-call tests for the transaction_detail entity (mirrors the
...
struct TransactionDetailDirectSetup {
...
fn transaction_detail_direct_setup(mockres: Value) -> TransactionDetailDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_TRANSACTION_DETAIL_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_TRANSACTION_DETAIL_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return TransactionDetailDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn transaction_detail_direct_load() {
    let setup = transaction_detail_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-transaction_detail", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["account01", "transaction01"] {
    ...
    if setup.live {
        setp(&params, "transaction_id", getp(&setup.idmap, "transaction01"));
    } else {
        setp(&params, "transaction_id", Value::str("direct02"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/payments/{transaction_id}")),
