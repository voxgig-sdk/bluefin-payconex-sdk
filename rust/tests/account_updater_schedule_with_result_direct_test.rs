// Generated direct-call tests for the account_updater_schedule_with_result entity (mirrors the
...
fn account_updater_schedule_with_result_direct_setup(mockres: Value) -> AccountUpdaterScheduleWithResultDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_SCHEDULE_WITH_RESULT_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_SCHEDULE_WITH_RESULT_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return AccountUpdaterScheduleWithResultDirectSetup {
            client,
            calls,
            live: true,
            idmap,
    ...
    AccountUpdaterScheduleWithResultDirectSetup {
        client,
        calls,
        live: false,
...
#[test]
fn account_updater_schedule_with_result_direct_list() {
    let setup = account_updater_schedule_with_result_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-account_updater_schedule_with_result", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["account01", "schedule01"] {
    ...
    if setup.live {
        setp(&params, "schedule_id", getp(&setup.idmap, "schedule01"));
    } else {
        setp(&params, "schedule_id", Value::str("direct02"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/account-updater/schedules/{schedule_id}/results")),
