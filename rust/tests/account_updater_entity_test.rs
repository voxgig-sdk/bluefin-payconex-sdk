...
#[test]
fn account_updater_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.account_updater(Value::Noval);
    assert_eq!(ent.get_name(), "account_updater");
}

#[test]
fn account_updater_entity_stream() {
    ...
    let plainent = plainsdk.account_updater(Value::Noval);
...
#[test]
fn account_updater_entity_basic() {
    let setup = account_updater_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in [] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("account_updater.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_ENTID JSON to run live");
        return;
    }

    // Bootstrap entity data from existing test data (no create step in flow).
    let account_updater_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "account_updater"], &setup.data)));
    let account_updater_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&account_updater_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
}

fn account_updater_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("account_updater");
    entity_data_file.push("AccountUpdaterTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("account_updater01"), Value::str("account_updater02"), Value::str("account_updater03"), Value::str("account01"), Value::str("account02"), Value::str("account03"), Value::str("subscription01"), Value::str("subscription02"), Value::str("subscription03"), Value::str("record01"), Value::str("record02"), Value::str("record03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_ENTID")) {
