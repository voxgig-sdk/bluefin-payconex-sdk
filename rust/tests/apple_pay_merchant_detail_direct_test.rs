// Generated direct-call tests for the apple_pay_merchant_detail entity (mirrors the
...
struct ApplePayMerchantDetailDirectSetup {
...
fn apple_pay_merchant_detail_direct_setup(mockres: Value) -> ApplePayMerchantDetailDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_APPLE_PAY_MERCHANT_DETAIL_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_APPLE_PAY_MERCHANT_DETAIL_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return ApplePayMerchantDetailDirectSetup {
            client,
            calls,
            live: true,
            idmap,
    ...
    ApplePayMerchantDetailDirectSetup {
        client,
        calls,
        live: false,
...
#[test]
fn apple_pay_merchant_detail_direct_list() {
    let setup = apple_pay_merchant_detail_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-apple_pay_merchant_detail", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/applePay/enrollment")),
