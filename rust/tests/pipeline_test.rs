...
use bluefin_payconex_sdk::core::helpers::{getp, getpath, ja, jo};
...
use bluefin_payconex_sdk::{
    ...
    BluefinPayconexSDK, Response, SdkResult, Spec, Utility, Value,
};

// plClient builds a client + isolated utility for pipeline utility tests.
fn pl_client(sdkopts: Value) -> (Rc<BluefinPayconexSDK>, Rc<Utility>) {
...
fn util_with(client: &Rc<BluefinPayconexSDK>, fetcher: FetcherFn) -> Rc<Utility> {
...
fn pipeline_feature_add_ordering_before_after_replace() {
    ...
    let names = |client: &Rc<BluefinPayconexSDK>| -> String {
        client
            .features
            .borrow()
            .iter()
...
use bluefin_payconex_sdk::core::helpers::get_str;
