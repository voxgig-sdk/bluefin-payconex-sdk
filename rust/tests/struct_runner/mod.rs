...
use bluefin_payconex_sdk::utility::voxgigstruct::ordered_map::OrderedMap;

use bluefin_payconex_sdk::utility::voxgigstruct::value::Value;
use bluefin_payconex_sdk::utility::voxgigstruct::*;
...
pub fn test_json() -> Value {
    ...
    bluefin_payconex_sdk::utility::jsonparse::json_parse(&txt).expect("parse test.json")
}
...
pub fn matchval(check: &Value, base: &Value) -> bool {
    ...
    if let Value::Str(cs) = check {
        ...
        if let Some(rem) = cs.strip_prefix('/').and_then(|s| s.strip_suffix('/')) {
            if let Ok(re) = bluefin_payconex_sdk::utility::voxgigstruct::re::Regex::new(rem) {
        ...
        for (i, entry) in entries.iter().enumerate() {
            ...
            match result {
                Err(msg) => {
                    ...
                    {
                        bluefin_payconex_sdk::utility::voxgigstruct::re::Regex::new(rem)
