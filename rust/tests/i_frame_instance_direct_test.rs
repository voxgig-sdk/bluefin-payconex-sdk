// Generated direct-call tests for the i_frame_instance entity (mirrors the
...
struct IFrameInstanceDirectSetup {
...
fn i_frame_instance_direct_setup(mockres: Value) -> IFrameInstanceDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_I_FRAME_INSTANCE_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_I_FRAME_INSTANCE_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return IFrameInstanceDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn i_frame_instance_direct_load() {
    let setup = i_frame_instance_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-i_frame_instance", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["account01", "i_frame_instance01", "payment_iframe01"] {
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "i_frame_instance01"));
    } else {
        setp(&params, "id", Value::str("direct02"));
    }
    if setup.live {
        setp(&params, "payment_iframe_id", getp(&setup.idmap, "payment_iframe01"));
    } else {
        setp(&params, "payment_iframe_id", Value::str("direct03"));
    }

    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/payment-iframe/{payment_iframe_id}/instance/{id}")),
