...
#[test]
fn payment_iframe_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.payment_iframe(Value::Noval);
    assert_eq!(ent.get_name(), "payment_iframe");
}

#[test]
fn payment_iframe_entity_stream() {
    ...
    let plainent = plainsdk.payment_iframe(Value::Noval);
...
#[test]
fn payment_iframe_entity_basic() {
    let setup = payment_iframe_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in [] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("payment_iframe.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_PAYMENT_IFRAME_ENTID JSON to run live");
        return;
    }

    // Bootstrap entity data from existing test data (no create step in flow).
    let payment_iframe_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "payment_iframe"], &setup.data)));
    let payment_iframe_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&payment_iframe_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
}

fn payment_iframe_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("payment_iframe");
    entity_data_file.push("PaymentIframeTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("payment_iframe01"), Value::str("payment_iframe02"), Value::str("payment_iframe03"), Value::str("account01"), Value::str("account02"), Value::str("account03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINPAYCONEX_TEST_PAYMENT_IFRAME_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_PAYMENT_IFRAME_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_PAYMENT_IFRAME_ENTID")) {
