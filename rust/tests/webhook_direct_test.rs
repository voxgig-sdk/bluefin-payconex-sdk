// Generated direct-call tests for the webhook entity (mirrors the
...
fn webhook_direct_setup(mockres: Value) -> WebhookDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return WebhookDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn webhook_direct_list() {
    let setup = webhook_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-webhook", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/webhooks")),
...
#[test]
fn webhook_direct_load() {
    let setup = webhook_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-webhook", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["account01", "webhook01"] {
            if getp(&setup.idmap, live_key).is_noval() {
                eprintln!("skip: live test needs {} via *_ENTID env var (synthetic IDs only)", live_key);
                return;
            }
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "webhook01"));
    } else {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/webhooks/{id}")),
            ("method", Value::str("GET")),
            ("params", params.clone()),
        ]))
        .expect("direct failed");
