...
#[test]
fn init_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.init(Value::Noval);
    assert_eq!(ent.get_name(), "init");
}

#[test]
fn init_entity_stream() {
    ...
    let plainent = plainsdk.init(Value::Noval);
...
#[test]
fn init_entity_basic() {
    let setup = init_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("init.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_INIT_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let init_ref01_ent = client.init(Value::Noval);
    let init_ref01_data = to_map(&getp(
        &getpath(&["new", "init"], &setup.data),
        "init_ref01",
    ));
    setp(&init_ref01_data, "account_id", getp(&setup.idmap, "account01"));

    let init_ref01_data_result = init_ref01_ent
        .create(init_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let init_ref01_data = to_map(&init_ref01_data_result);
    assert!(
        matches!(init_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn init_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("init");
    entity_data_file.push("InitTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("init01"), Value::str("init02"), Value::str("init03"), Value::str("account01"), Value::str("account02"), Value::str("account03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINPAYCONEX_TEST_INIT_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_INIT_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_INIT_ENTID")) {
