...
#[test]
fn apple_pay_session_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.apple_pay_session(Value::Noval);
    assert_eq!(ent.get_name(), "apple_pay_session");
}

#[test]
fn apple_pay_session_entity_stream() {
    ...
    let plainent = plainsdk.apple_pay_session(Value::Noval);
...
#[test]
fn apple_pay_session_entity_basic() {
    let setup = apple_pay_session_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("apple_pay_session.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let apple_pay_session_ref01_ent = client.apple_pay_session(Value::Noval);
    let apple_pay_session_ref01_data = to_map(&getp(
        &getpath(&["new", "apple_pay_session"], &setup.data),
        "apple_pay_session_ref01",
    ));
    setp(&apple_pay_session_ref01_data, "account_id", getp(&setup.idmap, "account01"));

    let apple_pay_session_ref01_data_result = apple_pay_session_ref01_ent
        .create(apple_pay_session_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let apple_pay_session_ref01_data = to_map(&apple_pay_session_ref01_data_result);
    assert!(
        matches!(apple_pay_session_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn apple_pay_session_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("apple_pay_session");
    entity_data_file.push("ApplePaySessionTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("apple_pay_session01"), Value::str("apple_pay_session02"), Value::str("apple_pay_session03"), Value::str("account01"), Value::str("account02"), Value::str("account03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID")) {
