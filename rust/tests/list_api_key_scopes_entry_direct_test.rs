// Generated direct-call tests for the list_api_key_scopes_entry entity (mirrors the
...
struct ListApiKeyScopesEntryDirectSetup {
...
fn list_api_key_scopes_entry_direct_setup(mockres: Value) -> ListApiKeyScopesEntryDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_LIST_API_KEY_SCOPES_ENTRY_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_LIST_API_KEY_SCOPES_ENTRY_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return ListApiKeyScopesEntryDirectSetup {
            client,
            calls,
            live: true,
            idmap,
    ...
    ListApiKeyScopesEntryDirectSetup {
        client,
        calls,
        live: false,
...
#[test]
fn list_api_key_scopes_entry_direct_list() {
    let setup = list_api_key_scopes_entry_direct_setup(ja(vec![
        jo(vec![("id", Value::str("direct01"))]),
        jo(vec![("id", Value::str("direct02"))]),
    ]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-list-list_api_key_scopes_entry", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/api-key-scopes")),
