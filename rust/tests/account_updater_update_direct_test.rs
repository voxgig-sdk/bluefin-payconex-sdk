// Generated direct-call tests for the account_updater_update entity (mirrors the
...
struct AccountUpdaterUpdateDirectSetup {
...
fn account_updater_update_direct_setup(mockres: Value) -> AccountUpdaterUpdateDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_UPDATE_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_UPDATE_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return AccountUpdaterUpdateDirectSetup {
            client,
            calls,
            live: true,
            idmap,
    ...
    AccountUpdaterUpdateDirectSetup {
        client,
        calls,
        live: false,
...
#[test]
fn account_updater_update_direct_load() {
    let setup = account_updater_update_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-account_updater_update", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["account01", "account_updater_update01"] {
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "account_updater_update01"));
    } else {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("api/v4/accounts/{account_id}/account-updater/updates/{id}")),
