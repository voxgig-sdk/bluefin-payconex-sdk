// dynamic_descriptor entity test (generated from the API model).

import XCTest

@testable import BluefinPayconexSdk

final class DynamicDescriptorEntityTest: XCTestCase {
  func testInstance() {
    let sdk = BluefinPayconexSDK.testSDK(nil, nil)
    let ent = sdk.DynamicDescriptor()
    XCTAssertEqual(ent.getName(), "dynamic_descriptor")
  }

  func testStream() async throws {
    // Seed two records (under the test feature's entity key) and activate
    // the streaming feature.
    let fixtures = vm(("entity", .map(vm(("dynamic_descriptor", .map(vm(
    ...
    let ent2 = sdk2.DynamicDescriptor()
    var streamed2: [Value] = []
