// apple_pay_merchant_detail entity test (generated from the API model).

import XCTest

@testable import BluefinPayconexSdk

final class ApplePayMerchantDetailEntityTest: XCTestCase {
  func testInstance() {
    let sdk = BluefinPayconexSDK.testSDK(nil, nil)
    let ent = sdk.ApplePayMerchantDetail()
    XCTAssertEqual(ent.getName(), "apple_pay_merchant_detail")
  }

  func testStream() async throws {
    // Seed two records (under the test feature's entity key) and activate
    // the streaming feature.
    let fixtures = vm(("entity", .map(vm(("apple_pay_merchant_detail", .map(vm(
    ...
    let ent2 = sdk2.ApplePayMerchantDetail()
    var streamed2: [Value] = []
