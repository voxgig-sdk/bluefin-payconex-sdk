// list_api_key_scopes_entry entity test (generated from the API model).

import XCTest

@testable import BluefinPayconexSdk

final class ListApiKeyScopesEntryEntityTest: XCTestCase {
  func testInstance() {
    let sdk = BluefinPayconexSDK.testSDK(nil, nil)
    let ent = sdk.ListApiKeyScopesEntry()
    XCTAssertEqual(ent.getName(), "list_api_key_scopes_entry")
  }

  func testStream() async throws {
    // Seed two records (under the test feature's entity key) and activate
    // the streaming feature.
    let fixtures = vm(("entity", .map(vm(("list_api_key_scopes_entry", .map(vm(
    ...
    let ent2 = sdk2.ListApiKeyScopesEntry()
    var streamed2: [Value] = []
