...
class WebhookEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $webhook_ref01_markdef_up0_name = "event";
...
function webhook_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["webhook01", "webhook02", "webhook03", "account01", "account02", "account03"] as $k) {
    ...
    $entid_env_raw = getenv("BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID"]);
