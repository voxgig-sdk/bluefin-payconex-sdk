...
// BluefinPayconex SDK pipeline test
//
...
{
    private static function utility(): BluefinPayconexUtility
    {
        return new BluefinPayconexUtility();
    }
    ...
    private static function ctx(array $over = []): BluefinPayconexContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BluefinPayconexContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BluefinPayconexOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BluefinPayconexError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BluefinPayconexOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BluefinPayconexMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BluefinPayconexOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BluefinPayconexMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BluefinPayconexOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BluefinPayconexMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BluefinPayconexSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BluefinPayconexMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BluefinPayconexResponse([]),
            'result' => new BluefinPayconexResult([]),
        ]);
        [, $err] = BluefinPayconexMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BluefinPayconexSpec([]),
            'response' => null,
            'result' => new BluefinPayconexResult([]),
        ]);
        [, $err] = BluefinPayconexMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BluefinPayconexSpec(['step' => 's']),
            'response' => new BluefinPayconexResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BluefinPayconexResult([]),
        ]);
        [, $err] = BluefinPayconexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BluefinPayconexSpec(['step' => 's']),
            'response' => new BluefinPayconexResponse(self::resp(200, ['v' => 1])),
            'result' => new BluefinPayconexResult([]),
        ]);
        [, $err] = BluefinPayconexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BluefinPayconexSpec(['step' => 's']),
            'response' => new BluefinPayconexResponse(self::resp(200, ['v' => 2])),
            'result' => new BluefinPayconexResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BluefinPayconexMakeResponse::call($ctx);
    ...
    {
        $preset = new BluefinPayconexResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BluefinPayconexSpec([]),
            'response' => new BluefinPayconexResponse([]),
            'result' => new BluefinPayconexResult([]),
        ...
        [$got, $err] = BluefinPayconexMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BluefinPayconexResult([])]);
        [, $err] = BluefinPayconexMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BluefinPayconexSpec([]), 'result' => null]);
        [, $err] = BluefinPayconexMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BluefinPayconexSpec(['step' => 's']),
            'result' => new BluefinPayconexResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BluefinPayconexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BluefinPayconexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BluefinPayconexSpec(['step' => 's']),
            'result' => new BluefinPayconexResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BluefinPayconexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BluefinPayconexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BluefinPayconexResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BluefinPayconexSpec([]),
            'result' => new BluefinPayconexResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BluefinPayconexMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BluefinPayconexMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BluefinPayconexError('boom', 'boom');
        $utility->fetcher = function (BluefinPayconexContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BluefinPayconexSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BluefinPayconexMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BluefinPayconexContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BluefinPayconexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BluefinPayconexResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BluefinPayconexMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BluefinPayconexContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BluefinPayconexSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BluefinPayconexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BluefinPayconexResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BluefinPayconexSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BluefinPayconexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BluefinPayconexMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BluefinPayconexMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BluefinPayconexResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BluefinPayconexDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BluefinPayconexResult(['ok' => false])]);
        $this->expectException(BluefinPayconexError::class);
        BluefinPayconexDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BluefinPayconexResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BluefinPayconexMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BluefinPayconexMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BluefinPayconexBaseFeature();
        $b = new BluefinPayconexBaseFeature();
        BluefinPayconexFeatureAdd::call($ctx, $a);
        BluefinPayconexFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BluefinPayconexBaseFeature
    {
        $f = new BluefinPayconexBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BluefinPayconexFeatureAdd::call($ctx, self::named_feature('a'));
        BluefinPayconexFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        BluefinPayconexFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        BluefinPayconexFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        BluefinPayconexFeatureAdd::call($ctx, $replace);
        ...
        BluefinPayconexFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BluefinPayconexContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BluefinPayconexSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BluefinPayconexPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        BluefinPayconexPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BluefinPayconexResponse(['headers' => null]),
            'result' => new BluefinPayconexResult([]),
        ]);
        BluefinPayconexResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        BluefinPayconexResultBody::call($ctx);
