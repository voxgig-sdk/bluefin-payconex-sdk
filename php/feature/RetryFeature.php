...
// BluefinPayconex SDK retry feature
...
class BluefinPayconexRetryFeature extends BluefinPayconexBaseFeature
{
    ...
    public function init(BluefinPayconexContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BluefinPayconexContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BluefinPayconexContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BluefinPayconexContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
