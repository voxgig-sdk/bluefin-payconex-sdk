# BluefinPayconex PHP SDK



The PHP SDK for the BluefinPayconex API — an entity-oriented client using PHP conventions.
...
```php
try {
    // load() returns the bare AccountUpdaterSubscriptionWithResult record (throws on error).
    ...
    print_r($accountupdatersubscriptionwithresult);
} catch (\Throwable $err) {
...
```php
// list() returns an array of AccountUpdaterScheduleWithResult records (throws on error).
...
```php
// load() returns the bare AccountUpdaterUpdate record (throws on error).
...
```php
// list() returns an array of ApplePayMerchantDetail records (throws on error).
...
```php
// load() returns the bare DynamicDescriptor record (throws on error).
...
```php
// list() returns an array of DynamicDescriptor records (throws on error).
...
```php
// load() returns the bare IFrameInstance record (throws on error).
...
```php
// load() returns the bare Iframe record (throws on error).
...
```php
// list() returns an array of Iframe records (throws on error).
...
```php
// list() returns an array of ListApiKeyScopesEntry records (throws on error).
...
```php
// load() returns the bare ThreeDSecureStatus record (throws on error).
...
```php
// load() returns the bare TransactionDetail record (throws on error).
...
```
php/
├── bluefinpayconex_sdk.php          -- Main SDK class
...
The main class (`bluefinpayconex_sdk.php`) exports the SDK class
