...
class BluefinPayconexParam
{
    public static function call(BluefinPayconexContext $ctx, mixed $paramdef): mixed
    {
        $point = $ctx->point;
        $spec = $ctx->spec;
        $match_val = $ctx->match;
        ...
        $akey = '';
        if ($point) {
            $alias_map = BluefinPayconexHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'alias'));
            if ($alias_map) {
