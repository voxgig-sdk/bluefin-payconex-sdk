...
func TestWebhookEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		webhookRef01MarkdefUp0Name := "event"
...
func webhookBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"webhook01", "webhook02", "webhook03", "account01", "account02", "account03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINPAYCONEX_TEST_WEBHOOK_ENTID"])
