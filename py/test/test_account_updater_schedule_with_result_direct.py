...
class TestAccountUpdaterScheduleWithResultDirect:

    def test_should_direct_list_account_updater_schedule_with_result(self):
        setup = _account_updater_schedule_with_result_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-account_updater_schedule_with_result", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["account01", "schedule01"]:
...
def _account_updater_schedule_with_result_direct_setup(mockres):
