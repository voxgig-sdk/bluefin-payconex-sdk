...
class TestInitEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Init(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _init_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "init." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINPAYCONEX_TEST_INIT_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        init_ref01_ent = client.Init(None)
        ...
        init_ref01_data = helpers.to_map(init_ref01_ent.create(init_ref01_data, None))
        assert init_ref01_data is not None



def _init_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/init/InitTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "BLUEFINPAYCONEX_TEST_INIT_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINPAYCONEX_TEST_INIT_ENTID"))
