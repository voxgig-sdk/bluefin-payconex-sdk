...
class TestTransactionDetailDirect:

    def test_should_direct_load_transaction_detail(self):
        setup = _transaction_detail_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-transaction_detail", "live" if setup["live"] else "unit")
        if _skip:
...
def _transaction_detail_direct_setup(mockres):
