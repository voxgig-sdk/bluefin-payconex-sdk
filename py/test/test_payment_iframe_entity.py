...
class TestPaymentIframeEntity:

    def test_should_create_instance(self):
        testsdk = BluefinPayconexSDK.test(None, None)
        ent = testsdk.PaymentIframe(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _payment_iframe_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "payment_iframe." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINPAYCONEX_TEST_PAYMENT_IFRAME_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        payment_iframe_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.payment_iframe")))
        payment_iframe_ref01_data = None
        if len(payment_iframe_ref01_data_raw) > 0:
            payment_iframe_ref01_data = helpers.to_map(payment_iframe_ref01_data_raw[0][1])



def _payment_iframe_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/payment_iframe/PaymentIframeTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "BLUEFINPAYCONEX_TEST_PAYMENT_IFRAME_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINPAYCONEX_TEST_PAYMENT_IFRAME_ENTID"))
