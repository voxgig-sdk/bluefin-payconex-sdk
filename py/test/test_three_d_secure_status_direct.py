...
class TestThreeDSecureStatusDirect:

    def test_should_direct_load_three_d_secure_status(self):
        setup = _three_d_secure_status_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-three_d_secure_status", "live" if setup["live"] else "unit")
        if _skip:
...
def _three_d_secure_status_direct_setup(mockres):
