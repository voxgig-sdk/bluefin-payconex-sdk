...
class TestAccountUpdaterUpdateDirect:

    def test_should_direct_load_account_updater_update(self):
        setup = _account_updater_update_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-account_updater_update", "live" if setup["live"] else "unit")
        if _skip:
...
def _account_updater_update_direct_setup(mockres):
