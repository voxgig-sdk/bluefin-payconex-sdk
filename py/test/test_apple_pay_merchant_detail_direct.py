...
class TestApplePayMerchantDetailDirect:

    def test_should_direct_list_apple_pay_merchant_detail(self):
        setup = _apple_pay_merchant_detail_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-apple_pay_merchant_detail", "live" if setup["live"] else "unit")
        if _skip:
...
def _apple_pay_merchant_detail_direct_setup(mockres):
