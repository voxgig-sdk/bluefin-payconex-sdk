...
class TestIFrameCreateInstanceEntity:

    def test_should_create_instance(self):
        testsdk = BluefinPayconexSDK.test(None, None)
        ent = testsdk.IFrameCreateInstance(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _i_frame_create_instance_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "i_frame_create_instance." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINPAYCONEX_TEST_I_FRAME_CREATE_INSTANCE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        i_frame_create_instance_ref01_ent = client.IFrameCreateInstance(None)
        ...
        i_frame_create_instance_ref01_data = helpers.to_map(i_frame_create_instance_ref01_ent.create(i_frame_create_instance_ref01_data, None))
        assert i_frame_create_instance_ref01_data is not None



def _i_frame_create_instance_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/i_frame_create_instance/IFrameCreateInstanceTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINPAYCONEX_TEST_I_FRAME_CREATE_INSTANCE_ENTID"))
