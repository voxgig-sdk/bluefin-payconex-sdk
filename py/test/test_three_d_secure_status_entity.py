...
class TestThreeDSecureStatusEntity:

    def test_should_create_instance(self):
        testsdk = BluefinPayconexSDK.test(None, None)
        ent = testsdk.ThreeDSecureStatus(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _three_d_secure_status_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "three_d_secure_status." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINPAYCONEX_TEST_THREE_D_SECURE_STATUS_ENTID JSON to run live")
        client = setup["client"]
        ...
        three_d_secure_status_ref01_data = None
        if len(three_d_secure_status_ref01_data_raw) > 0:
            three_d_secure_status_ref01_data = helpers.to_map(three_d_secure_status_ref01_data_raw[0][1])

        # LOAD
        three_d_secure_status_ref01_ent = client.ThreeDSecureStatus(None)
        three_d_secure_status_ref01_match_dt0 = {}
        three_d_secure_status_ref01_data_dt0_loaded = three_d_secure_status_ref01_ent.load(three_d_secure_status_ref01_match_dt0, None)
        assert three_d_secure_status_ref01_data_dt0_loaded is not None



def _three_d_secure_status_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/three_d_secure_status/ThreeDSecureStatusTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINPAYCONEX_TEST_THREE_D_SECURE_STATUS_ENTID"))
