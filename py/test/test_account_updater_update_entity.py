...
class TestAccountUpdaterUpdateEntity:

    def test_should_create_instance(self):
        testsdk = BluefinPayconexSDK.test(None, None)
        ent = testsdk.AccountUpdaterUpdate(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _account_updater_update_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "account_updater_update." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_UPDATE_ENTID JSON to run live")
        client = setup["client"]
        ...
        account_updater_update_ref01_data = None
        if len(account_updater_update_ref01_data_raw) > 0:
            account_updater_update_ref01_data = helpers.to_map(account_updater_update_ref01_data_raw[0][1])

        # LOAD
        account_updater_update_ref01_ent = client.AccountUpdaterUpdate(None)
        account_updater_update_ref01_match_dt0 = {
            "id": account_updater_update_ref01_data["id"],
        }
        account_updater_update_ref01_data_dt0_loaded = account_updater_update_ref01_ent.load(account_updater_update_ref01_match_dt0, None)
        account_updater_update_ref01_data_dt0_load_result = helpers.to_map(account_updater_update_ref01_data_dt0_loaded)
        assert account_updater_update_ref01_data_dt0_load_result is not None
        assert account_updater_update_ref01_data_dt0_load_result["id"] == account_updater_update_ref01_data["id"]



def _account_updater_update_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/account_updater_update/AccountUpdaterUpdateTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_UPDATE_ENTID"))
