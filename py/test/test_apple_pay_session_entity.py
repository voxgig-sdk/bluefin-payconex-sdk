...
class TestApplePaySessionEntity:

    def test_should_create_instance(self):
        testsdk = BluefinPayconexSDK.test(None, None)
        ent = testsdk.ApplePaySession(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _apple_pay_session_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "apple_pay_session." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        apple_pay_session_ref01_ent = client.ApplePaySession(None)
        ...
        apple_pay_session_ref01_data = helpers.to_map(apple_pay_session_ref01_ent.create(apple_pay_session_ref01_data, None))
        assert apple_pay_session_ref01_data is not None



def _apple_pay_session_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/apple_pay_session/ApplePaySessionTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFINPAYCONEX_TEST_APPLE_PAY_SESSION_ENTID"))
