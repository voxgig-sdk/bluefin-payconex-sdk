...
class TransactionDetailEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TransactionDetailEntity(self._client, opts)
    ...
    def data_get(self) -> TransactionDetail:
    ...
    def match_get(self) -> TransactionDetail:
    ...
    def load(self, reqmatch=None, ctrl=None) -> TransactionDetail:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TransactionDetail().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: TransactionDetailUpdateData, ctrl=None) -> TransactionDetail:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
