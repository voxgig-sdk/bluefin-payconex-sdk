...
class AccountUpdaterEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AccountUpdaterEntity(self._client, opts)
    ...
    def data_get(self) -> AccountUpdater:
    ...
    def match_get(self) -> AccountUpdater:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> AccountUpdater:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.AccountUpdater().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
