...
class PaymentIframeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PaymentIframeEntity(self._client, opts)
    ...
    def data_get(self) -> PaymentIframe:
    ...
    def match_get(self) -> PaymentIframe:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PaymentIframe:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PaymentIframe().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
