...
class IFrameInstanceEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IFrameInstanceEntity(self._client, opts)
    ...
    def data_get(self) -> IFrameInstance:
    ...
    def match_get(self) -> IFrameInstance:
    ...
    def load(self, reqmatch=None, ctrl=None) -> IFrameInstance:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.IFrameInstance().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
