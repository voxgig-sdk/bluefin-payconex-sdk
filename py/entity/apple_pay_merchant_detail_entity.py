...
class ApplePayMerchantDetailEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ApplePayMerchantDetailEntity(self._client, opts)
    ...
    def data_get(self) -> ApplePayMerchantDetail:
    ...
    def match_get(self) -> ApplePayMerchantDetail:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ApplePayMerchantDetail]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ApplePayMerchantDetail().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: ApplePayMerchantDetailCreateData, ctrl=None) -> ApplePayMerchantDetail:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
