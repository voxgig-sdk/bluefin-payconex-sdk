// BluefinPayconex SDK — network-simulation tests over the offline mock transport
...
static void offline_simulation_fails_request() {
  auto client = BluefinPayconexSDK::testSDK(fhMap({{"net", fhMap({{"offline", Value(true)}})}}), Value::undef());
...
static void latency_simulation_delays_request() {
  int delay = 60;
  auto client = BluefinPayconexSDK::testSDK(fhMap({{"net", fhMap({{"latency", Value(delay)}})}}), Value::undef());
