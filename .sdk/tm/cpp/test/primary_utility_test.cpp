// BluefinPayconex SDK — drives the primary utility functions against the shared
...
static std::shared_ptr<BluefinPayconexSDK> client() { return BluefinPayconexSDK::testSDK(); }

// Helper: create basic test context.
static CtxPtr makeTestCtx(std::shared_ptr<BluefinPayconexSDK> client_, UtilityPtr utility) {
  CtxSpec cs;
  cs.setOpname("load");
...
static CtxPtr makeTestFullCtx(std::shared_ptr<BluefinPayconexSDK> client_, UtilityPtr utility) {
...
static void fetcherLive() {
  ...
  auto liveClient = std::make_shared<BluefinPayconexSDK>(opts);
...
static void fetcherBlockedTestMode() {
  ...
  Value opts = fhMap({{"system", fhMap({{"fetch", Value(fetchFn)}})}});
  auto blockedClient = std::make_shared<BluefinPayconexSDK>(opts);
...
static void makeSpecBasic() {
  ...
  auto specClient = BluefinPayconexSDK::testSDK(Value::undef(), setupOpts);
...
static void prepareAuthBasic() {
  ...
  auto authClient = BluefinPayconexSDK::testSDK(Value::undef(), setupOpts);
