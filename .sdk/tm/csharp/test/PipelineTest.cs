...
public class PipelineTest
{
    ...
    private static (BluefinPayconexSDK, Utility) PlClient(Dictionary<string, object?>? sdkopts)
    {
        var client = BluefinPayconexSDK.TestSDK(null, sdkopts);
        return (client, client.GetUtility());
    }

    private static Context PlCtx(BluefinPayconexSDK client, Utility utility,
    ...
    private static string ErrCode(Action act)
    {
        ...
        catch (BluefinPayconexError err)
        {
            return err.Code;
        }
        catch (Exception)
    ...
    private static Utility UtilWith(BluefinPayconexSDK client, FetcherFunc fetcher)
    {
    ...
    public void MakeRequestTransportErrorCarriedOnResponse()
    {
        ...
        Assert.True(resp.Err is BluefinPayconexError pe && pe.Code == "boom",
