import '../BluefinPayconexError.dart';
import '../Result.dart';
...
dynamic makeError(dynamic ctx, [dynamic err]) {
  ...
  final msg = 'BluefinPayconexSDK: ' + opname.toString() + ': ' + errmsg(err);

  BluefinPayconexError sdkerr;
  if (err is BluefinPayconexError) {
    sdkerr = err;
    sdkerr.message = clean(ctx, msg);
  } else {
    final code = errcode(err);
    sdkerr = BluefinPayconexError('' == code ? 'unknown' : code, clean(ctx, msg), ctx);
  }
