# BluefinPayconex SDK generic entity behaviour
#
...
defmodule BluefinPayconex.EntityBase do
  alias Voxgig.Struct, as: S
  alias BluefinPayconex.Helpers, as: H
  alias BluefinPayconex.{Context, Utility, Pipeline}
  ...
  def construct(module, client, name, entopts \\ nil) do
    ...
    S.setprop(ent, "_utility", BluefinPayconex.get_utility(client))
    S.setprop(ent, "_entopts", entopts)
    S.setprop(ent, "_data", S.jm([]))
    S.setprop(ent, "_match", S.jm([]))

    entctx =
      Context.new(S.jm(["entity", ent, "entopts", entopts]), BluefinPayconex.get_root_ctx(client))
