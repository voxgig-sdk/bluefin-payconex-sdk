...
// BluefinPayconex SDK feature test
//
...
class FtCtrl extends BluefinPayconexControl
{
...
class FtClient
{
    ...
    private BluefinPayconexUtility $utility;

    public function __construct(BluefinPayconexUtility $utility, array $options)
    {
    ...
    public function get_utility(): BluefinPayconexUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BluefinPayconexContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BluefinPayconexContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BluefinPayconexContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BluefinPayconexConfig::make_feature($name);
    ...
    private static function buildUrl(BluefinPayconexSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BluefinPayconexError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BluefinPayconexResponse($fetched);
        }
    ...
    private function fail(BluefinPayconexContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BluefinPayconexContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
