...
class BluefinPayconexTestFeature extends BluefinPayconexBaseFeature
{
    ...
    public function init(BluefinPayconexContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BluefinPayconexContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BluefinPayconexParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BluefinPayconexContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BluefinPayconexContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BluefinPayconexParam::call($ctx, $k);
