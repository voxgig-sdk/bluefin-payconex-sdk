...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = BluefinPayconexBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = BluefinPayconexSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = BluefinPayconexSDK.test(nil, nil)
  ...
  def test_fetcher_live
    calls = []
    live_client = BluefinPayconexSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = BluefinPayconexSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
        },
  ...
  def test_make_spec_basic
    ...
    spec_client = BluefinPayconexSDK.test(nil, setup_opts)
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = BluefinPayconexOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = BluefinPayconexSDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = BluefinPayconexContext.new(ctxmap, nil)
  ...
  def err_from_map(m)
    ...
    code = m["code"] || ""
    BluefinPayconexError.new(code, msg)
  end
end
