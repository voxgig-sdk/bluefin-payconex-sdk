...
pub fn make_error_util(ctx: *Context) E!Value {
    ...
    const the_err: *err.BluefinPayconexError = in_err orelse (result.err orelse ctx.make_error("unknown", "unknown error"));

    const errmsg = the_err.msg;
    const msg0 = fmt("BluefinPayconexSDK: {s}: {s}", .{ opname, errmsg });
    ...
    const sdk_err = err.BluefinPayconexError.make("", msg);
