...
final class PipelineTest: XCTestCase {
  ...
  private func plClient(_ sdkopts: VMap?) -> (BluefinPayconexSDK, Utility) {
    let client = BluefinPayconexSDK.testSDK(nil, sdkopts)
    return (client, client.getUtility())
  }

  private func plCtx(_ client: BluefinPayconexSDK, _ utility: Utility, _ ctrl: VMap?) -> Context {
  ...
  private func errCode(_ act: () throws -> Void) -> String {
    do { try act(); return "" } catch let e as BluefinPayconexError { return e.code } catch { return "" }
  }
  ...
  private func utilWith(_ client: BluefinPayconexSDK, _ fetcher: @escaping FetcherFunc) -> Utility {
  ...
  func testMakeRequestTransportErrorCarriedOnResponse() {
    ...
    XCTAssertEqual((resp.err as? BluefinPayconexError)?.code, "boom")
  }
