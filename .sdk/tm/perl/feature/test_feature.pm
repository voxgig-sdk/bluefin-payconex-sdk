# BluefinPayconex SDK test feature
...
package BluefinPayconexTestFeature;
...
sub init {
  ...
  my $entity = BluefinPayconexHelpers::gp($options, 'entity');
  ...
  my $test_fetcher = sub {
    ...
    my $op = $fctx->{op};
    my $entmap = BluefinPayconexHelpers::gp($entity, $op->{entity});
    ...
    my $resolve_match = sub {
      ...
      for my $src ($fctx->{match}, $fctx->{data}) {
        ...
        my $v = BluefinPayconexHelpers::gp($src, 'id');
    ...
    if ('load' eq $op->{name}) {
      ...
      my $ent = BluefinPayconexHelpers::ge($found, 0);
      return $respond->(404, undef, { 'statusText' => 'Not found' })
        unless BluefinPayconexHelpers::rb_truthy($ent);
      ...
      if (Voxgig::Struct::ismap($fctx->{reqdata})) {
        ...
        if ($op->{alias}) {
          my $alias_id = BluefinPayconexHelpers::gp($op->{alias}, 'id');
      ...
      if (Voxgig::Struct::ismap($ent)) {
        my $id = BluefinPayconexHelpers::gp($ent, 'id');
  ...
  my $net = BluefinPayconexHelpers::gp($options, 'net');
...
sub make_netsim {
  ...
  my $do_sleep = sub {
    ...
    if (ref $net->{sleep} eq 'CODE') {
      ...
      BluefinPayconexHelpers::sleep_ms($ms);
    }
  };
  ...
  return sub {
    ...
    if (BluefinPayconexHelpers::is_true($net->{offline})) {
...
sub build_args {
  ...
  my $points = BluefinPayconexHelpers::gpath($ctx->{config},
    "entity.$entname.op.$opname.points");
  my $point = BluefinPayconexHelpers::ge($points, -1);

  my $params_path = BluefinPayconexHelpers::gpath($point, 'args.params');
    ...
    if ($keys) {
      for my $key (@$keys) {
        ...
        if ($is_id || $is_reqd) {
          ...
          my $ka = $op->{alias} ? BluefinPayconexHelpers::gp($op->{alias}, $key) : undef;
