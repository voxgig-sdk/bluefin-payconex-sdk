...
{
  my $sdk = BluefinPayconexSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = BluefinPayconexSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = BluefinPayconexSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = BluefinPayconexSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = BluefinPayconexSDK->test(undef, undef);
