...
sub has_feature {
  my ($name) = @_;
  my $f = BluefinPayconexConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = BluefinPayconexHelpers::to_map(
    BluefinPayconexHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = BluefinPayconexHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = BluefinPayconexHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { BluefinPayconexHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = BluefinPayconexSDK->test({ 'entity' => $seed }, undef);
  ...
  if (has_feature('streaming')) {
    {
      my $sdk = BluefinPayconexSDK->test(
        { 'entity' => $seed },
        { 'feature' => { 'streaming' => {
