# BluefinPayconex SDK utility: make_point
...
$REGISTRY{make_point} = sub {
  ...
  if ($ctx->{out}{point}) {
    ...
    return (undef, $preset)
      if Scalar::Util::blessed($preset) && $preset->isa('BluefinPayconexError');
    $ctx->{point} = $preset;
  ...
  my $allow_op = BluefinPayconexHelpers::gpath($options, 'allow.op');
  ...
  if (1 == @{ $op->{points} }) {
    ...
    for my $p (@{ $op->{points} }) {
      $point = $p;
      my $select_def = BluefinPayconexHelpers::to_map(BluefinPayconexHelpers::gp($p, 'select'));
      my $found = 1;

      if ($selector && $select_def) {
        my $exist = BluefinPayconexHelpers::gp($select_def, 'exist');
        if (Voxgig::Struct::islist($exist)) {
          for my $ek (@$exist) {
            my $rv = BluefinPayconexHelpers::gp($reqselector, "$ek");
            my $sv = BluefinPayconexHelpers::gp($selector, "$ek");
      ...
      if ($found) {
        my $req_action = BluefinPayconexHelpers::gp($reqselector, '$action');
        my $select_action = BluefinPayconexHelpers::gp($select_def, '$action');
        $found = 0 unless BluefinPayconexHelpers::eqv($req_action, $select_action);
      }
    ...
    if ($reqselector) {
      ...
      if (defined $req_action && $point) {
        my $point_select = BluefinPayconexHelpers::to_map(BluefinPayconexHelpers::gp($point, 'select'));
        my $point_action = BluefinPayconexHelpers::gp($point_select, '$action');
        unless (BluefinPayconexHelpers::eqv($req_action, $point_action)) {
