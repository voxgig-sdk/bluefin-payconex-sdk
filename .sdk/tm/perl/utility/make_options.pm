# BluefinPayconex SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinPayconexHelpers::gp($options, 'utility');
  ...
  my @featureorder;
  my $feature_raw = BluefinPayconexHelpers::gp($opts, 'feature');
  if (Voxgig::Struct::islist($feature_raw)) {
    my %fmap;
    for my $entry (@$feature_raw) {
      next unless Voxgig::Struct::ismap($entry);
      my $name = BluefinPayconexHelpers::gp($entry, 'name');
  ...
  BluefinPayconexHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinPayconexHelpers::gpath($opts, 'system.fetch');
  ...
  my $clean_keys = BluefinPayconexHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinPayconexHelpers::to_map(
      BluefinPayconexHelpers::gp($opts, 'feature')) || {};
