# BluefinPayconex SDK utility: prepare_query
...
$REGISTRY{prepare_query} = sub {
  ...
  my $params = [];
  if ($point) {
    my $p = BluefinPayconexHelpers::gp($point, 'params');
  ...
  if ($items) {
    for my $item (@$items) {
      my ($key, $val) = @$item;
      next unless BluefinPayconexHelpers::rb_truthy($val) && defined $key && !ref $key;
