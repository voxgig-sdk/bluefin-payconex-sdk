# BluefinPayconex SDK utility: fetcher
...
our $DefaultHttpFetch = sub {
  ...
  $hdrs{'User-Agent'} = 'Mozilla/5.0 (compatible; BluefinPayconexSDK/1.0)' unless $has_ua;
...
$REGISTRY{fetcher} = sub {
  ...
  if (BluefinPayconexHelpers::is_true(BluefinPayconexHelpers::gpath($options, 'feature.test.active'))) {
    return (undef, $ctx->make_error('fetch_test_block',
      "Request blocked as test feature is active (URL was: \"$fullurl\")"));
  }

  my $sys_fetch = BluefinPayconexHelpers::gpath($options, 'system.fetch');
