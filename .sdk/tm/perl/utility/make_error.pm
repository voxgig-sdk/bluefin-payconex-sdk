...
$REGISTRY{make_error} = sub {
  my ($ctx, $err) = @_;
  if (!defined $ctx) {
    $ctx = BluefinPayconexContext->new({}, undef);
  }
  my $op = $ctx->{op} || BluefinPayconexOperation->new({});
  my $opname = $op->{name};
  $opname = 'unknown operation' if !defined $opname || '' eq $opname || '_' eq $opname;

  my $result = $ctx->{result} || BluefinPayconexResult->new({});
  $result->{ok} = 0;

  $err = $result->{err} if !defined $err;
  $err = $ctx->make_error('unknown', 'unknown error') if !defined $err;

  my $errmsg = (Scalar::Util::blessed($err) && $err->isa('BluefinPayconexError'))
    ? $err->{msg} : "$err";
  $errmsg =~ s/\s+\z//;
  my $msg = "BluefinPayconexSDK: $opname: $errmsg";
  ...
  my $sdk_err = BluefinPayconexError->new('', $msg, $ctx);
