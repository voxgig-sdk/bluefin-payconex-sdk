...
package BluefinPayconexContext;
...
sub new {
  ...
  my $gcp = \&BluefinPayconexHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinPayconexControl->new();
  ...
  $self->{data} = BluefinPayconexHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinPayconexHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinPayconexHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinPayconexHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinPayconexSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinPayconexResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinPayconexResponse'))
...
sub resolve_op {
  ...
  return BluefinPayconexOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinPayconexHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinPayconexHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinPayconexOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinPayconexError->new($code, $msg, $self);
}
