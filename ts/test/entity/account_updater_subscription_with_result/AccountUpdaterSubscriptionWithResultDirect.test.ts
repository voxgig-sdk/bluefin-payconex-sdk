...
describe('AccountUpdaterSubscriptionWithResultDirect', async () => {
  ...
  test('direct-load-account_updater_subscription_with_result', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-account_updater_subscription_with_result', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFINPAYCONEX_TEST_ACCOUNT_UPDATER_SUBSCRIPTION_WITH_RESULT_ENTID']
