...
describe('IframeDirect', async () => {
  ...
  test('direct-load-iframe', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-iframe', setup.live)) return
  ...
  test('direct-list-iframe', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-iframe', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BLUEFINPAYCONEX_TEST_IFRAME_ENTID']
