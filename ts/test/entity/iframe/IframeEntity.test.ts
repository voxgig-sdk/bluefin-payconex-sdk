...
describe('IframeEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list', 'update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'iframe.' + op, live)) return
    }
    ...
    // UPDATE
    const iframe_ref01_data_up0: any = {}
    ...
    ;(iframe_ref01_data_up0 as any)[iframe_ref01_markdef_up0.name] = iframe_ref01_markdef_up0.value
    ...
    assert((iframe_ref01_resdata_up0 as any)[iframe_ref01_markdef_up0.name] === iframe_ref01_markdef_up0.value)


    // LOAD
    const iframe_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_PAYCONEX_TEST_IFRAME_ENTID']
