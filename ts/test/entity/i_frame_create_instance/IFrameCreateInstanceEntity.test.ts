...
describe('IFrameCreateInstanceEntity', async () => {
  ...
  test('instance', async () => {
    ...
    const ent = testsdk.IFrameCreateInstance()
    assert(null != ent)
  })
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'i_frame_create_instance.' + op, live)) return
    }
    ...
    // CREATE
    const i_frame_create_instance_ref01_ent = client.IFrameCreateInstance()
    let i_frame_create_instance_ref01_data = setup.data.new.i_frame_create_instance['i_frame_create_instance_ref01']
    ...
    assert(null != i_frame_create_instance_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_PAYCONEX_TEST_I_FRAME_CREATE_INSTANCE_ENTID']
