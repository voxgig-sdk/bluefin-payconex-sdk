...
describe('PaymentIframeEntity', async () => {
  ...
  test('instance', async () => {
    ...
    const ent = testsdk.PaymentIframe()
    assert(null != ent)
  })
  ...
  test('basic', async (t) => {
    ...
    for (const op of []) {
      if (maybeSkipControl(t, 'entityOp', 'payment_iframe.' + op, live)) return
    }
    ...
    let payment_iframe_ref01_data = Object.values(setup.data.existing.payment_iframe)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_PAYCONEX_TEST_PAYMENT_IFRAME_ENTID']
