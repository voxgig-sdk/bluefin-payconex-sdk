...
describe('AccountUpdaterScheduleWithResultEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'list']) {
      if (maybeSkipControl(t, 'entityOp', 'account_updater_schedule_with_result.' + op, live)) return
    }
    ...
    // LIST
    const account_updater_schedule_with_result_ref01_match: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_PAYCONEX_TEST_ACCOUNT_UPDATER_SCHEDULE_WITH_RESULT_ENTID']
