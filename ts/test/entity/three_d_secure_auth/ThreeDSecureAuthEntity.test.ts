...
describe('ThreeDSecureAuthEntity', async () => {
  ...
  test('instance', async () => {
    ...
    const ent = testsdk.ThreeDSecureAuth()
    assert(null != ent)
  })
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'three_d_secure_auth.' + op, live)) return
    }
    ...
    // CREATE
    const three_d_secure_auth_ref01_ent = client.ThreeDSecureAuth()
    let three_d_secure_auth_ref01_data = setup.data.new.three_d_secure_auth['three_d_secure_auth_ref01']
    ...
    assert(null != three_d_secure_auth_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID']
