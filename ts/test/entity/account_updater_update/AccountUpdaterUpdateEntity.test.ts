...
describe('AccountUpdaterUpdateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'account_updater_update.' + op, live)) return
    }
    ...
    let account_updater_update_ref01_data = Object.values(setup.data.existing.account_updater_update)[0] as any

    // LOAD
    const account_updater_update_ref01_ent = client.AccountUpdaterUpdate()
    const account_updater_update_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_PAYCONEX_TEST_ACCOUNT_UPDATER_UPDATE_ENTID']
