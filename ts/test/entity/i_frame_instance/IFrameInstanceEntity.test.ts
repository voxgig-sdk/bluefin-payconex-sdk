...
describe('IFrameInstanceEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'i_frame_instance.' + op, live)) return
    }
    ...
    let i_frame_instance_ref01_data = Object.values(setup.data.existing.i_frame_instance)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['BLUEFIN_PAYCONEX_TEST_I_FRAME_INSTANCE_ENTID']
