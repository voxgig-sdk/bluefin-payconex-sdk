...
test('stream', (t) async {
      ...
      final plainent = plainsdk.Webhook();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create', 'list', 'update', 'load', 'remove']) {
        if (maybeSkipControl(t, 'entityOp', 'webhook.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_PAYCONEX_TEST_WEBHOOK_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final webhook_ref01_ent = client.Webhook();
      dynamic webhook_ref01_data = setup['data']['new']['webhook']['webhook_ref01'];
      webhook_ref01_data['account_id'] = setup['idmap']['account01'];

      webhook_ref01_data = await webhook_ref01_ent.create(webhook_ref01_data);
      ok(null != webhook_ref01_data['id']);


      // LIST
      final webhook_ref01_match = <String, dynamic>{};
      webhook_ref01_match['account_id'] = setup['idmap']['account01'];

      final webhook_ref01_list = await webhook_ref01_ent.list(webhook_ref01_match);

      ok(!isempty(select(
          (webhook_ref01_list as List).map((e) => e.data()).toList(),
          {'id': webhook_ref01_data['id']})));


      // UPDATE
      final webhook_ref01_data_up0 = <String, dynamic>{};
      webhook_ref01_data_up0['id'] = webhook_ref01_data['id'];
      webhook_ref01_data_up0['account_id'] = setup['idmap']['account_id'];

      final webhook_ref01_markdef_up0 = <String, dynamic>{
        'name': 'event',
        'value': 'Mark01-webhook_ref01_' + setup['now'].toString(),
      };
      webhook_ref01_data_up0[webhook_ref01_markdef_up0['name']] = webhook_ref01_markdef_up0['value'];

      final webhook_ref01_resdata_up0 = await webhook_ref01_ent.update(webhook_ref01_data_up0);
      ok(webhook_ref01_resdata_up0['id'] == webhook_ref01_data_up0['id']);

      ok(webhook_ref01_resdata_up0[webhook_ref01_markdef_up0['name']] == webhook_ref01_markdef_up0['value']);


      // LOAD
      final webhook_ref01_match_dt0 = <String, dynamic>{};
      webhook_ref01_match_dt0['id'] = webhook_ref01_data['id'];
      final webhook_ref01_data_dt0 = await webhook_ref01_ent.load(webhook_ref01_match_dt0);
      ok(webhook_ref01_data_dt0['id'] == webhook_ref01_data['id']);


      // REMOVE
      final webhook_ref01_match_rm0 = <String, dynamic>{'id': webhook_ref01_data['id']};
      await webhook_ref01_ent.remove(webhook_ref01_match_rm0);


      // LIST
      final webhook_ref01_match_rt0 = <String, dynamic>{};
      webhook_ref01_match_rt0['account_id'] = setup['idmap']['account01'];

      final webhook_ref01_list_rt0 = await webhook_ref01_ent.list(webhook_ref01_match_rt0);

      ok(isempty(select(
          (webhook_ref01_list_rt0 as List).map((e) => e.data()).toList(),
          {'id': webhook_ref01_data['id']})));
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['webhook01', 'webhook02', 'webhook03', 'account01', 'account02', 'account03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_PAYCONEX_TEST_WEBHOOK_ENTID'];
