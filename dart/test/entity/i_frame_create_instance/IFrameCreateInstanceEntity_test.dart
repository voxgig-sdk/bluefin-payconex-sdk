...
void tests() {
  describe('IFrameCreateInstanceEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinPayconexSDK.test();
      final ent = testsdk.IFrameCreateInstance();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.IFrameCreateInstance();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'i_frame_create_instance.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_PAYCONEX_TEST_I_FRAME_CREATE_INSTANCE_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final i_frame_create_instance_ref01_ent = client.IFrameCreateInstance();
      dynamic i_frame_create_instance_ref01_data = setup['data']['new']['i_frame_create_instance']['i_frame_create_instance_ref01'];
      ...
      i_frame_create_instance_ref01_data = await i_frame_create_instance_ref01_ent.create(i_frame_create_instance_ref01_data);
      ok(null != i_frame_create_instance_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['i_frame_create_instance01', 'i_frame_create_instance02', 'i_frame_create_instance03', 'account01', 'account02', 'account03', 'payment_iframe01', 'payment_iframe02', 'payment_iframe03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_PAYCONEX_TEST_I_FRAME_CREATE_INSTANCE_ENTID'];
  final idmapOverridden =
      null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');

  final env = envOverride({
    'BLUEFIN_PAYCONEX_TEST_I_FRAME_CREATE_INSTANCE_ENTID': idmap,
    'BLUEFIN_PAYCONEX_TEST_LIVE': 'FALSE',
    'BLUEFIN_PAYCONEX_TEST_EXPLAIN': 'FALSE',
    'BLUEFIN_PAYCONEX_APIKEY': 'NONE',
  });

  idmap = env['BLUEFIN_PAYCONEX_TEST_I_FRAME_CREATE_INSTANCE_ENTID'];
