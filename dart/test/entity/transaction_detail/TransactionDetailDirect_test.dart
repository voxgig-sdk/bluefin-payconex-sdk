...
void tests() {
  describe('TransactionDetailDirect', () {
    ...
    test('direct-load-transaction_detail', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-transaction_detail', true == setup['live'])) {
        return;
      }
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFINPAYCONEX_TEST_TRANSACTION_DETAIL_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFINPAYCONEX_TEST_TRANSACTION_DETAIL_ENTID'];
