...
void tests() {
  describe('ThreeDSecureStatusEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinPayconexSDK.test();
      final ent = testsdk.ThreeDSecureStatus();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.ThreeDSecureStatus();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['load']) {
        if (maybeSkipControl(t, 'entityOp', 'three_d_secure_status.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_STATUS_ENTID JSON to run live');
        return;
      }
      ...
      final three_d_secure_status_ref01_data =
          (setup['data']['existing']['three_d_secure_status'] as Map).values.first;

      // LOAD: skipped — no entity id field and load requires path params.
      // Entity-var is declared here so later flow steps still compile.
      final three_d_secure_status_ref01_ent = client.ThreeDSecureStatus();
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['three_d_secure_status01', 'three_d_secure_status02', 'three_d_secure_status03', 'account01', 'account02', 'account03', '3_d01', '3_d02', '3_d03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_STATUS_ENTID'];
  final idmapOverridden =
      null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');

  final env = envOverride({
    'BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_STATUS_ENTID': idmap,
    'BLUEFIN_PAYCONEX_TEST_LIVE': 'FALSE',
    'BLUEFIN_PAYCONEX_TEST_EXPLAIN': 'FALSE',
    'BLUEFIN_PAYCONEX_APIKEY': 'NONE',
  });

  idmap = env['BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_STATUS_ENTID'];
