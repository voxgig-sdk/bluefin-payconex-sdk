...
void tests() {
  describe('AccountUpdaterEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinPayconexSDK.test();
      final ent = testsdk.AccountUpdater();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.AccountUpdater();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in []) {
        if (maybeSkipControl(t, 'entityOp', 'account_updater.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_PAYCONEX_TEST_ACCOUNT_UPDATER_ENTID JSON to run live');
        return;
      }
      ...
      final account_updater_ref01_data =
          (setup['data']['existing']['account_updater'] as Map).values.first;
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['account_updater01', 'account_updater02', 'account_updater03', 'account01', 'account02', 'account03', 'subscription01', 'subscription02', 'subscription03', 'account01', 'account02', 'account03', 'subscription01', 'subscription02', 'subscription03', 'record01', 'record02', 'record03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_PAYCONEX_TEST_ACCOUNT_UPDATER_ENTID'];
  final idmapOverridden =
      null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');

  final env = envOverride({
    'BLUEFIN_PAYCONEX_TEST_ACCOUNT_UPDATER_ENTID': idmap,
    'BLUEFIN_PAYCONEX_TEST_LIVE': 'FALSE',
    'BLUEFIN_PAYCONEX_TEST_EXPLAIN': 'FALSE',
    'BLUEFIN_PAYCONEX_APIKEY': 'NONE',
  });

  idmap = env['BLUEFIN_PAYCONEX_TEST_ACCOUNT_UPDATER_ENTID'];
