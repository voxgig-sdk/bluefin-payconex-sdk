...
void tests() {
  describe('PaymentIframeEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinPayconexSDK.test();
      final ent = testsdk.PaymentIframe();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.PaymentIframe();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in []) {
        if (maybeSkipControl(t, 'entityOp', 'payment_iframe.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_PAYCONEX_TEST_PAYMENT_IFRAME_ENTID JSON to run live');
        return;
      }
      ...
      final payment_iframe_ref01_data =
          (setup['data']['existing']['payment_iframe'] as Map).values.first;
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['payment_iframe01', 'payment_iframe02', 'payment_iframe03', 'account01', 'account02', 'account03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
