...
void tests() {
  describe('AccountUpdaterScheduleEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinPayconexSDK.test();
      final ent = testsdk.AccountUpdaterSchedule();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.AccountUpdaterSchedule();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'account_updater_schedule.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_PAYCONEX_TEST_ACCOUNT_UPDATER_SCHEDULE_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final account_updater_schedule_ref01_ent = client.AccountUpdaterSchedule();
      dynamic account_updater_schedule_ref01_data = setup['data']['new']['account_updater_schedule']['account_updater_schedule_ref01'];
      ...
      ok(null != account_updater_schedule_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['account_updater_schedule01', 'account_updater_schedule02', 'account_updater_schedule03', 'account01', 'account02', 'account03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_PAYCONEX_TEST_ACCOUNT_UPDATER_SCHEDULE_ENTID'];
