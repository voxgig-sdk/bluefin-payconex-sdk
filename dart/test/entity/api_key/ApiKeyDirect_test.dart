...
void tests() {
  describe('ApiKeyDirect', () {
    ...
    test('direct-load-api_key', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-api_key', true == setup['live'])) {
        return;
      }
    ...
    test('direct-list-api_key', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-api_key', true == setup['live'])) {
        return;
      }
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFINPAYCONEX_TEST_API_KEY_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFINPAYCONEX_TEST_API_KEY_ENTID'];
