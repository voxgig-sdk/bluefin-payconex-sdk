...
void tests() {
  describe('DynamicDescriptorDirect', () {
    ...
    test('direct-load-dynamic_descriptor', (t) async {
      final setup = directSetup({'id': 'direct01'});
      if (maybeSkipControl(
          t, 'direct', 'direct-load-dynamic_descriptor', true == setup['live'])) {
        return;
      }
    ...
    test('direct-list-dynamic_descriptor', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-dynamic_descriptor', true == setup['live'])) {
        return;
      }
      if (skipIfMissingIds(t, setup, ["account01"])) {
        return;
      }
      final client = setup['client'];
      final calls = setup['calls'];
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFINPAYCONEX_TEST_DYNAMIC_DESCRIPTOR_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFINPAYCONEX_TEST_DYNAMIC_DESCRIPTOR_ENTID'];
