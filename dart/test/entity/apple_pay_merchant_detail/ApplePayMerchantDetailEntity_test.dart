...
void tests() {
  describe('ApplePayMerchantDetailEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinPayconexSDK.test();
      final ent = testsdk.ApplePayMerchantDetail();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.ApplePayMerchantDetail();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create', 'list']) {
        if (maybeSkipControl(t, 'entityOp', 'apple_pay_merchant_detail.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final apple_pay_merchant_detail_ref01_ent = client.ApplePayMerchantDetail();
      dynamic apple_pay_merchant_detail_ref01_data = setup['data']['new']['apple_pay_merchant_detail']['apple_pay_merchant_detail_ref01'];
      ...
      ok(null != apple_pay_merchant_detail_ref01_data);


      // LIST
      final apple_pay_merchant_detail_ref01_match = <String, dynamic>{};
      apple_pay_merchant_detail_ref01_match['account_id'] = setup['idmap']['account01'];

      final apple_pay_merchant_detail_ref01_list = await apple_pay_merchant_detail_ref01_ent.list(apple_pay_merchant_detail_ref01_match);

      ok(!isempty(select(
          (apple_pay_merchant_detail_ref01_list as List).map((e) => e.data()).toList(),
          {'id': apple_pay_merchant_detail_ref01_data['id']})));
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['apple_pay_merchant_detail01', 'apple_pay_merchant_detail02', 'apple_pay_merchant_detail03', 'account01', 'account02', 'account03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_PAYCONEX_TEST_APPLE_PAY_MERCHANT_DETAIL_ENTID'];
