...
void tests() {
  describe('ListApiKeyScopesEntryDirect', () {
    ...
    test('direct-list-list_api_key_scopes_entry', (t) async {
      final setup = directSetup([
        {'id': 'direct01'},
        {'id': 'direct02'}
      ]);
      if (maybeSkipControl(
          t, 'direct', 'direct-list-list_api_key_scopes_entry', true == setup['live'])) {
        return;
      }
...
Map<String, dynamic> directSetup([dynamic mockres]) {
  ...
  final env = envOverride({
    'BLUEFINPAYCONEX_TEST_LIST_API_KEY_SCOPES_ENTRY_ENTID': <String, dynamic>{},
  ...
  if (live) {
    ...
    dynamic idmap = env['BLUEFINPAYCONEX_TEST_LIST_API_KEY_SCOPES_ENTRY_ENTID'];
