...
void tests() {
  describe('ThreeDSecureAuthEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinPayconexSDK.test();
      final ent = testsdk.ThreeDSecureAuth();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.ThreeDSecureAuth();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'three_d_secure_auth.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final three_d_secure_auth_ref01_ent = client.ThreeDSecureAuth();
      dynamic three_d_secure_auth_ref01_data = setup['data']['new']['three_d_secure_auth']['three_d_secure_auth_ref01'];
      three_d_secure_auth_ref01_data['3_d_id'] = setup['idmap']['3_d01'];
      three_d_secure_auth_ref01_data['account_id'] = setup['idmap']['account01'];

      three_d_secure_auth_ref01_data = await three_d_secure_auth_ref01_ent.create(three_d_secure_auth_ref01_data);
      ok(null != three_d_secure_auth_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['three_d_secure_auth01', 'three_d_secure_auth02', 'three_d_secure_auth03', 'account01', 'account02', 'account03', '3_d01', '3_d02', '3_d03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID'];
  final idmapOverridden =
      null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');

  final env = envOverride({
    'BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID': idmap,
    'BLUEFIN_PAYCONEX_TEST_LIVE': 'FALSE',
    'BLUEFIN_PAYCONEX_TEST_EXPLAIN': 'FALSE',
    'BLUEFIN_PAYCONEX_APIKEY': 'NONE',
  });

  idmap = env['BLUEFIN_PAYCONEX_TEST_THREE_D_SECURE_AUTH_ENTID'];
