...
DIRECT_LOAD: {
  my $setup = transaction_detail_direct_setup({ 'id' => 'direct01' });
  my ($_should_skip, $_reason) = BluefinPayconexTestRunner::is_control_skipped(
    'direct', 'direct-load-transaction_detail', $setup->{live} ? 'live' : 'unit');
  if ($_should_skip) {
    note($_reason || 'skipped via sdk-test-control.json');
    pass('direct-load-transaction_detail: skipped via sdk-test-control.json');
    last DIRECT_LOAD;
  }
  ...
  if ($setup->{live}) {
    ...
    if (defined $result->{err}) {
      ...
      pass('direct-load-transaction_detail: skipped (live)');
      last DIRECT_LOAD;
    }
    unless ($result->{ok}) {
    ...
    pass('direct-load-transaction_detail: live ok');
  }
  else {
    ok(!defined $result->{err}, 'direct-load-transaction_detail: no error');
    ok($result->{ok}, 'direct-load-transaction_detail: ok');
    is(BluefinPayconexHelpers::to_int($result->{status}), 200, 'direct-load-transaction_detail: status');
    ok(defined $result->{data}, 'direct-load-transaction_detail: data');
    if (Voxgig::Struct::ismap($result->{data})) {
      is($result->{data}{id}, 'direct01', 'direct-load-transaction_detail: id');
    }
    is(scalar @{ $setup->{calls} }, 1, 'direct-load-transaction_detail: 1 call');
  }
}


sub transaction_detail_direct_setup {
  my ($mockres) = @_;
