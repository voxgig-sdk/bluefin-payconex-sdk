...

{
  ...
  our @ISA = ('BluefinPayconexBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinPayconexBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub make_ctx_from_map {
  ...
  my $ctx = BluefinPayconexContext->new($ctxmap, undef);
  ...
  if (Voxgig::Struct::ismap($ctxmap->{response})) {
    ...
    $ctx->{response} = BluefinPayconexResponse->new($resp_map);
    if (BluefinPayconexHelpers::rb_truthy($resp_map->{body})) {
...
sub err_from_map {
  ...
  return BluefinPayconexError->new($code, $msg);
}
...

{
  ...
  my $feature = BluefinPayconexBaseFeature->new;
...
{
  my $hook_client = BluefinPayconexSDK->test(undef, undef);
...
{
  my $init_client = BluefinPayconexSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinPayconexSDK->new({
    'system' => {
      'fetch' => sub {
        my ($url, $fetchdef) = @_;
...
{
  my $blocked_client = BluefinPayconexSDK->new({
    'system' => {
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinPayconexSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...

{
  ...
  my $spec_client = BluefinPayconexSDK->test(undef, $setup_opts);
...
runset('makeUrl.basic', get_spec($PRIMARY, 'makeUrl', 'basic'), sub {
  ...
  my $ctxmap = $entry->{ctx} || {};
  my $ctx = make_ctx_from_map($ctxmap, $client, $utility);
  $ctx->{result} = BluefinPayconexResult->new({}) unless $ctx->{result};
...
runset('operator.basic', get_spec($PRIMARY, 'operator', 'basic'), sub {
  my ($entry) = @_;
  my $in_val = $entry->{in} || {};
  my $op = BluefinPayconexOperation->new($in_val);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...

{
  ...
  my $auth_client = BluefinPayconexSDK->test(undef, $setup_opts);
