...
sub resp {
  ...
  return BluefinPayconexResponse->new({
    'status' => $status,
...
sub spec_of {
  my (%map) = @_;
  return BluefinPayconexSpec->new({
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('BluefinPayconexError');
}
...

{
  ...
  $ctx->{result} = BluefinPayconexResult->new({});
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = BluefinPayconexResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = BluefinPayconexResult->new({ 'ok' => 1 });
...
{
  my $boom = BluefinPayconexError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, BluefinPayconexError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinPayconexResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinPayconexResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = BluefinPayconexResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = BluefinPayconexResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = BluefinPayconexSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = BluefinPayconexBaseFeature->new;
  my $b = BluefinPayconexBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = BluefinPayconexBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(BluefinPayconexHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(BluefinPayconexHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = BluefinPayconexContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? BluefinPayconexSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinPayconexResponse->new({ 'status' => 200 });
  $ctx->{result} = BluefinPayconexResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinPayconexResponse->new({
