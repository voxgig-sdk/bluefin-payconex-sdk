...
package BluefinPayconexTestRunner;
...
sub env_override {
  my ($m) = @_;
  my $live = getenv('BLUEFINPAYCONEX_TEST_LIVE');
  my $override = getenv('BLUEFINPAYCONEX_TEST_OVERRIDE');
  ...
  my $explain = getenv('BLUEFINPAYCONEX_TEST_EXPLAIN');
  $m->{'BLUEFINPAYCONEX_TEST_EXPLAIN'} = $explain if defined $explain && '' ne $explain;
...
sub is_control_skipped {
  ...
  my $skip = BluefinPayconexHelpers::gpath($ctrl, "test.skip.$mode") || {};
  my $items = $skip->{$kind} || [];
...
sub live_delay_ms {
  my $ctrl = load_test_control();
  my $v = BluefinPayconexHelpers::gpath($ctrl, 'test.live.delayMs');
