...
# operation pipeline. $features is a list of { name =>, options => }.
{
  ...
  sub new {
    ...
    $self->{utility} = BluefinPayconexUtility->new;
    ...
    for my $fspec (@$features) {
      ...
      my $f = BluefinPayconexFeatures::make_feature($name);
  ...
  sub op {
    ...
    my $out = eval {
      ...
      if (Scalar::Util::blessed($ctx->{out}{point})
        && $ctx->{out}{point}->isa('BluefinPayconexError')) {
        die $ctx->{out}{point};
      }
      ...
      $ctx->{response} = Voxgig::Struct::ismap($fetched)
        ? BluefinPayconexResponse->new($fetched) : undef;
    ...
    if (my $err = $@) {
      if (Scalar::Util::blessed($err) && $err->isa('BluefinPayconexError')) {
        $ctx->{ctrl}{err} = $err;
  ...
  sub populate_result {
    ...
    my $result = BluefinPayconexResult->new({});
    $ctx->{result} = $result;
...
if (has_feature('retry')) {
...

  {
    ...
    my ($server, $calls) = recording_server(sub {
      my ($n) = @_;
      return $n < 3
        ? (undef, BluefinPayconexError->new('boom', 'boom'))
