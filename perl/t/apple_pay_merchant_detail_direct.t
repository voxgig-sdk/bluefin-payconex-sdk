#!perl
# ApplePayMerchantDetail direct test
...
DIRECT_LIST: {
  my $setup = apple_pay_merchant_detail_direct_setup([
    { 'id' => 'direct01' },
    { 'id' => 'direct02' },
  ]);
  my ($_should_skip, $_reason) = BluefinPayconexTestRunner::is_control_skipped(
    'direct', 'direct-list-apple_pay_merchant_detail', $setup->{live} ? 'live' : 'unit');
  if ($_should_skip) {
    note($_reason || 'skipped via sdk-test-control.json');
    pass('direct-list-apple_pay_merchant_detail: skipped via sdk-test-control.json');
    last DIRECT_LIST;
  }
  if ($setup->{live}) {
    for my $_live_key ('account01') {
      if (!defined $setup->{idmap}{$_live_key}) {
        ...
        pass('direct-list-apple_pay_merchant_detail: skipped');
        last DIRECT_LIST;
      }
    }
  }
  ...
  if ($setup->{live}) {
    ...
    if (defined $result->{err}) {
      ...
      pass('direct-list-apple_pay_merchant_detail: skipped (live)');
      last DIRECT_LIST;
    }
    unless ($result->{ok}) {
    ...
    pass('direct-list-apple_pay_merchant_detail: live ok');
  }
  else {
    ok(!defined $result->{err}, 'direct-list-apple_pay_merchant_detail: no error');
    ok($result->{ok}, 'direct-list-apple_pay_merchant_detail: ok');
    is(BluefinPayconexHelpers::to_int($result->{status}), 200, 'direct-list-apple_pay_merchant_detail: status');
    ok(Voxgig::Struct::islist($result->{data}), 'direct-list-apple_pay_merchant_detail: data is array');
    is(scalar @{ $result->{data} }, 2, 'direct-list-apple_pay_merchant_detail: data length');
    is(scalar @{ $setup->{calls} }, 1, 'direct-list-apple_pay_merchant_detail: 1 call');
  }
}


sub apple_pay_merchant_detail_direct_setup {
  my ($mockres) = @_;
