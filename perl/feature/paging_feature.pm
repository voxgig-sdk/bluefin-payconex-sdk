# BluefinPayconex SDK paging feature
#
...
package BluefinPayconexPagingFeature;
...
sub init {
  ...
  $self->{active} = BluefinPayconexHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreResult {
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = BluefinPayconexHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = BluefinPayconexHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = BluefinPayconexHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (BluefinPayconexHelpers::is_true($bhasmore) || BluefinPayconexHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = BluefinPayconexHelpers::is_true($bhasmore) ? 1 : 0;
    }
  }
