# BluefinPayconex SDK Init entity
...
sub make {
  ...
  return InitEntity->new($self->{_client}, $opts);
}
...
# Returns the current Init data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Init fields).
sub match_get {
  my ($self) = @_;
...
# Create a new Init.
#
# reqdata: body data hashref (InitCreateData shape). ctrl: optional
# per-call control. Returns the created Init data (hashref); dies
