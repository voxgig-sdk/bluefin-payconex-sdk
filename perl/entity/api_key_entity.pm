# BluefinPayconex SDK ApiKey entity
...
# Returns the current ApiKey data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ApiKey fields).
sub match_get {
  my ($self) = @_;
...
# Load a single ApiKey.
#
# reqmatch: match criteria hashref (id/query fields; ApiKeyLoadMatch
...
# Returns the loaded ApiKey data (hashref); dies with BluefinPayconexError
# on failure.
sub load {
...
# List ApiKey items matching the given filter.
#
# reqmatch: match filter hashref (any subset of ApiKey fields;
# ApiKeyListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching ApiKey items as an arrayref; dies with
...
# Create a new ApiKey.
#
# reqdata: body data hashref (ApiKeyCreateData shape). ctrl: optional
# per-call control. Returns the created ApiKey data (hashref); dies
...
#
# reqdata: body data hashref (ApiKeyUpdateData shape). ctrl: optional
# per-call control. Returns the updated ApiKey data (hashref); dies
# with BluefinPayconexError on failure.
sub update {
  my ($self, $reqdata, $ctrl) = @_;
  my $utility = $self->{_utility};
...
# Remove an ApiKey matching the given criteria.
#
# reqmatch: match criteria hashref (id/query fields; ApiKeyRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed ApiKey
