...
package DynamicDescriptorEntity;
...
sub new {
  ...
  elsif (BluefinPayconexHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
...
sub make {
  ...
  return DynamicDescriptorEntity->new($self->{_client}, $opts);
}

sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinPayconexHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current DynamicDescriptor data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinPayconexHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of DynamicDescriptor fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; DynamicDescriptorLoadMatch
...
# Returns the loaded DynamicDescriptor data (hashref); dies with BluefinPayconexError
# on failure.
sub load {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      ...
      if ($result->{resdata}) {
        $self->{_data} = BluefinPayconexHelpers::to_map(
...
#
# reqmatch: match filter hashref (any subset of DynamicDescriptor fields;
# DynamicDescriptorListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching DynamicDescriptor items as an arrayref; dies with
# BluefinPayconexError on failure.
sub list {
  my ($self, $reqmatch, $ctrl) = @_;
  my $utility = $self->{_utility};
  my $ctx = $utility->{make_context}->({
...
#
# reqdata: body data hashref (DynamicDescriptorCreateData shape). ctrl: optional
# per-call control. Returns the created DynamicDescriptor data (hashref); dies
# with BluefinPayconexError on failure.
sub create {
...
#
# reqdata: body data hashref (DynamicDescriptorUpdateData shape). ctrl: optional
# per-call control. Returns the updated DynamicDescriptor data (hashref); dies
# with BluefinPayconexError on failure.
sub update {
  my ($self, $reqdata, $ctrl) = @_;
  my $utility = $self->{_utility};
...
#
# reqmatch: match criteria hashref (id/query fields; DynamicDescriptorRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed DynamicDescriptor
# data (hashref); dies with BluefinPayconexError on failure.
sub remove {
  my ($self, $reqmatch, $ctrl) = @_;
  my $utility = $self->{_utility};
  my $ctx = $utility->{make_context}->({
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinPayconexHelpers::to_map($callopts) || {};
  my $signal = BluefinPayconexHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinPayconexHelpers::to_map(
    BluefinPayconexHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinPayconexHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinPayconexHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinPayconexHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
