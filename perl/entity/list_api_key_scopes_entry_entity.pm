...
package ListApiKeyScopesEntryEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'list_api_key_scopes_entry',
    _client => $client,
...
sub make {
  ...
  return ListApiKeyScopesEntryEntity->new($self->{_client}, $opts);
}
...
# Returns the current ListApiKeyScopesEntry data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ListApiKeyScopesEntry fields).
sub match_get {
  my ($self) = @_;
...
# List ListApiKeyScopesEntry items matching the given filter.
#
# reqmatch: match filter hashref (any subset of ListApiKeyScopesEntry fields;
# ListApiKeyScopesEntryListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching ListApiKeyScopesEntry items as an arrayref; dies with
