...
sub make {
  ...
  return IFrameInstanceEntity->new($self->{_client}, $opts);
}
...
# Returns the current IFrameInstance data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of IFrameInstance fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; IFrameInstanceLoadMatch
...
# Returns the loaded IFrameInstance data (hashref); dies with BluefinPayconexError
# on failure.
sub load {
