...
sub make {
  ...
  return PaymentIframeEntity->new($self->{_client}, $opts);
}
...
# Returns the current PaymentIframe data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of PaymentIframe fields).
sub match_get {
  my ($self) = @_;
...
# Remove an PaymentIframe matching the given criteria.
#
# reqmatch: match criteria hashref (id/query fields; PaymentIframeRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed PaymentIframe
