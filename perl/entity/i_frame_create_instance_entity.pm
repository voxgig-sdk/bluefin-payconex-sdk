...
package IFrameCreateInstanceEntity;
...
sub make {
  ...
  return IFrameCreateInstanceEntity->new($self->{_client}, $opts);
}
...
# Returns the current IFrameCreateInstance data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of IFrameCreateInstance fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (IFrameCreateInstanceCreateData shape). ctrl: optional
# per-call control. Returns the created IFrameCreateInstance data (hashref); dies
