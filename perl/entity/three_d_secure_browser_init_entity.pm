...
package ThreeDSecureBrowserInitEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'three_d_secure_browser_init',
    _client => $client,
...
sub make {
  ...
  return ThreeDSecureBrowserInitEntity->new($self->{_client}, $opts);
}
...
# Returns the current ThreeDSecureBrowserInit data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ThreeDSecureBrowserInit fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (ThreeDSecureBrowserInitCreateData shape). ctrl: optional
# per-call control. Returns the created ThreeDSecureBrowserInit data (hashref); dies
