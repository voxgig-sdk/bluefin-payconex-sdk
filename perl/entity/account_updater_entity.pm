# BluefinPayconex SDK AccountUpdater entity
...
sub make {
  ...
  return AccountUpdaterEntity->new($self->{_client}, $opts);
}
...
# Returns the current AccountUpdater data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of AccountUpdater fields).
sub match_get {
  my ($self) = @_;
...
# Remove an AccountUpdater matching the given criteria.
#
# reqmatch: match criteria hashref (id/query fields; AccountUpdaterRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed AccountUpdater
