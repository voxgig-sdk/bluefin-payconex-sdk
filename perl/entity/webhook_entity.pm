# BluefinPayconex SDK Webhook entity
...
# Returns the current Webhook data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Webhook fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; WebhookLoadMatch
...
# Returns the loaded Webhook data (hashref); dies with BluefinPayconexError
# on failure.
sub load {
...
#
# reqmatch: match filter hashref (any subset of Webhook fields;
# WebhookListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Webhook items as an arrayref; dies with
...
#
# reqdata: body data hashref (WebhookCreateData shape). ctrl: optional
# per-call control. Returns the created Webhook data (hashref); dies
...
#
# reqdata: body data hashref (WebhookUpdateData shape). ctrl: optional
# per-call control. Returns the updated Webhook data (hashref); dies
# with BluefinPayconexError on failure.
sub update {
  my ($self, $reqdata, $ctrl) = @_;
  my $utility = $self->{_utility};
...
#
# reqmatch: match criteria hashref (id/query fields; WebhookRemoveMatch
# shape). ctrl: optional per-call control. Returns the removed Webhook
