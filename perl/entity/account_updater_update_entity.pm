...
package AccountUpdaterUpdateEntity;
...
sub make {
  ...
  return AccountUpdaterUpdateEntity->new($self->{_client}, $opts);
}
...
# Returns the current AccountUpdaterUpdate data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of AccountUpdaterUpdate fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; AccountUpdaterUpdateLoadMatch
...
# Returns the loaded AccountUpdaterUpdate data (hashref); dies with BluefinPayconexError
# on failure.
sub load {
