...
package ApplePayMerchantDetailEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'apple_pay_merchant_detail',
    _client => $client,
...
sub make {
  ...
  return ApplePayMerchantDetailEntity->new($self->{_client}, $opts);
}
...
# Returns the current ApplePayMerchantDetail data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ApplePayMerchantDetail fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match filter hashref (any subset of ApplePayMerchantDetail fields;
# ApplePayMerchantDetailListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching ApplePayMerchantDetail items as an arrayref; dies with
...
#
# reqdata: body data hashref (ApplePayMerchantDetailCreateData shape). ctrl: optional
# per-call control. Returns the created ApplePayMerchantDetail data (hashref); dies
