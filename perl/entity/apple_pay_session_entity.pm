# BluefinPayconex SDK ApplePaySession entity
...
sub make {
  ...
  return ApplePaySessionEntity->new($self->{_client}, $opts);
}
...
# Returns the current ApplePaySession data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ApplePaySession fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (ApplePaySessionCreateData shape). ctrl: optional
# per-call control. Returns the created ApplePaySession data (hashref); dies
