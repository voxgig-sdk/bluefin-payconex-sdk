# BluefinPayconex SDK ThreeDSecureAuth entity
...
package ThreeDSecureAuthEntity;
...
sub make {
  ...
  return ThreeDSecureAuthEntity->new($self->{_client}, $opts);
}
...
# Returns the current ThreeDSecureAuth data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ThreeDSecureAuth fields).
sub match_get {
  my ($self) = @_;
...
# Create a new ThreeDSecureAuth.
#
# reqdata: body data hashref (ThreeDSecureAuthCreateData shape). ctrl: optional
# per-call control. Returns the created ThreeDSecureAuth data (hashref); dies
