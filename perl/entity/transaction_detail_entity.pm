...
package TransactionDetailEntity;
...
sub make {
  ...
  return TransactionDetailEntity->new($self->{_client}, $opts);
}
...
# Returns the current TransactionDetail data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of TransactionDetail fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; TransactionDetailLoadMatch
...
# Returns the loaded TransactionDetail data (hashref); dies with BluefinPayconexError
# on failure.
sub load {
...
#
# reqdata: body data hashref (TransactionDetailUpdateData shape). ctrl: optional
# per-call control. Returns the updated TransactionDetail data (hashref); dies
