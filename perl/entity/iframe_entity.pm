...
sub make {
  ...
  return IframeEntity->new($self->{_client}, $opts);
}
...
# Returns the current Iframe data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Iframe fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; IframeLoadMatch
...
# Returns the loaded Iframe data (hashref); dies with BluefinPayconexError
# on failure.
sub load {
...
#
# reqmatch: match filter hashref (any subset of Iframe fields;
# IframeListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Iframe items as an arrayref; dies with
...
# Create a new Iframe.
#
# reqdata: body data hashref (IframeCreateData shape). ctrl: optional
# per-call control. Returns the created Iframe data (hashref); dies
...
#
# reqdata: body data hashref (IframeUpdateData shape). ctrl: optional
# per-call control. Returns the updated Iframe data (hashref); dies
# with BluefinPayconexError on failure.
sub update {
  my ($self, $reqdata, $ctrl) = @_;
  my $utility = $self->{_utility};
