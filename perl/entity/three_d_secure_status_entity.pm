...
package ThreeDSecureStatusEntity;
...
sub make {
  ...
  return ThreeDSecureStatusEntity->new($self->{_client}, $opts);
}
...
# Returns the current ThreeDSecureStatus data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of ThreeDSecureStatus fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match criteria hashref (id/query fields; ThreeDSecureStatusLoadMatch
...
# Returns the loaded ThreeDSecureStatus data (hashref); dies with BluefinPayconexError
# on failure.
sub load {
