...
package AccountUpdaterScheduleEntity;
...
sub new {
  ...
  my $self = bless {
    _name => 'account_updater_schedule',
    _client => $client,
...
sub make {
  ...
  return AccountUpdaterScheduleEntity->new($self->{_client}, $opts);
}
...
# Returns the current AccountUpdaterSchedule data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of AccountUpdaterSchedule fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (AccountUpdaterScheduleCreateData shape). ctrl: optional
# per-call control. Returns the created AccountUpdaterSchedule data (hashref); dies
