# BluefinPayconex SDK utility: prepare_auth
...
$REGISTRY{prepare_auth} = sub {
  ...
  if (!defined BluefinPayconexHelpers::gp($options, 'auth')) {
  ...
  if (!defined $apikey || Voxgig::Struct::is_none($apikey)
    ...
    my $auth_prefix = BluefinPayconexHelpers::gpath($options, 'auth.prefix');
