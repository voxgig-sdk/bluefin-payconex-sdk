...
$BluefinPayconexUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %BluefinPayconexUtilities::REGISTRY) {
    $u->{$k} = $BluefinPayconexUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = BluefinPayconexHelpers::struct_facade();
};
